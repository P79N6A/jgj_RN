    ...
    render() {
        return (
            ...
            >
                ...
                <TouchableOpacity activeOpacity={.7} style={{ height: 264, backgroundColor: '#ebebeb' }}>
                    {/* title */}
                    ...
                    {/* 选项 */}
                    {
                        this.state.selectarr.map((item, key) => {
                            return (
                                <TouchableOpacity activeOpacity={.7} onPress={() => this.props.selectwork(item.name)} key={key} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 53, backgroundColor: "#fff", marginBottom: 1 }}>
