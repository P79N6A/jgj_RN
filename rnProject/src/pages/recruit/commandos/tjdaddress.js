 ...
 * @Date: 2019-03-25 15:51:59 
 * @Module:招突击队选择城市
 * @Last Modified time: 2019-03-25 15:51:59 
 */
...
export default class lookworker extends Component {
    ...
    clickaddress(obj) {
        ...
        this.state.addressone.map((item, key) => {
            if (item.key == obj.a) {
                GLOBAL.tjdaddress.tjdoneName = item.name
            }
        })
        this.state.addresstwo.map((item, key) => {
            if (item.key == obj.b) {
                GLOBAL.tjdaddress.tjdtwoName = item.name
            }
        })
        GLOBAL.tjdaddress.tjdone = obj.a
        GLOBAL.tjdaddress.tjdtwo = obj.b
