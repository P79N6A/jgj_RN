...
WCDB_SYNTHESIZE(JGJChatMsgListModel, extend)
...
- (NSAttributedString *)htmlStr {
    ...
    NSString * htmlString;
    if (self.chatListType == JGJChatListIntegralType) {
...
- (NSString *)msg_type{
    ...
    }else if (self.chatListType == JGJChatListUpgradeGroupChatType) { //群升级为班组或者项目组
...
- (CGFloat)cellHeight {
    ...
    }else if ([self.msg_type isEqualToString:@"local_verified"]) {
...
- (CGFloat)getWorkingTypeMsgHeight {
    ...
    if (self.msg_total_type == JGJChatActivityMsgType) { //活动消息高度
        ...
        }else if (self.chatListType == JGJChatListWorkGroupChatType || self.chatListType == JGJChatListLocalGroupChatType) {
        ...
        if (self.chatListType == JGJChatListQuality || self.chatListType == JGJChatListSafe || self.chatListType == JGJChatListNotice || self.chatListType == JGJChatListMeeting || self.chatListType == JGJChatListLog || self.chatListType == JGJChatListTaskType || self.chatListType == JGJChatListInspectType || self.chatListType == JGJChatListApproveType || self.chatListType == JGJChatListMeeting || self.chatListType == JGJChatListOssType || self.chatListType ==  JGJChatListFriendType) {// 质量 安全等通知类型 25 为cell底部查看详情按钮高度
            ...
            if (self.chatListType == JGJChatListDemandSyncBillType || self.chatListType == JGJChatListSyncBillToYouType || self.chatListType == JGJChatListagreeSyncBillType || self.chatListType == JGJChatListCancellSyncBillType || self.chatListType == JGJChatListRefuseSyncBillType) {
                
                height = 0.0;
            }
        }else if (self.chatListType == JGJChatListJoinType || self.chatListType == JGJChatListReopenType || self.chatListType == JGJChatListRemoveType || self.chatListType == JGJChatListCloseType || self.chatListType == JGJChatListSwitchgroupType || self.chatListType == JGJChatListEvaluateType || self.chatListType == JGJChatListDismissGroupType) {// 加入新班组类型
...
//            height = ceil(size.height) + 1 + 10 + 80 + 5;
        ...
        if ([self.class_type isEqualToString:@"group"] || [self.origin_class_type isEqualToString:@"group"]) {
        ...
        if (self.chatListType == JGJChatListBottomDefaultSpaceType) {// 底部空白cell
            ...
            height = [NSString stringWithYYContentWidth:TYGetUIScreenWidth - 100 font:AppFont32Size lineSpace:4 content:self.detail] + 100 + 24;
        }
