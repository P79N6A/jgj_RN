...
- (void)subClassWithModel:(JGJLogSectionListModel *)jlogSectionModel{
    
}
-(void)subLogClassWithModel:(JGJLogSectionListModel *)jgjChatListModel
{
    _nameCnterConstace.constant = 0;
    //心需求 去掉时间
...
//    }
    ...
    //发施工日志情况，优先显示质量，再显示
//    if (![NSString isEmpty:jgjChatListModel.show_list_content]) {
...
#pragma mark - 修改过后可以显示设定的行数
        //        self.contentLabelConstraintH.constant = labelheight > 36?36:labelheight;
        /*
        if (imgCount<=0) {
//            self.contentLabelConstraintH.constant = labelheight>37?74:37;
            self.contentLabelConstraintH.constant = labelheight>37?40:37;

        }else{
            self.contentLabelConstraintH.constant = 40;
        }
        */
...
- (void)subClassSetWithModel:(JGJChatMsgListModel *)jgjChatListModel{
    ...
    //设置头像
//    [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:[JLGHttpRequest_IP stringByAppendingString:self.jgjChatListModel.head_pic]] placeholderImage:[UIImage imageNamed:@"defaultHead_Man"] options:SDWebImageRefreshCached];
