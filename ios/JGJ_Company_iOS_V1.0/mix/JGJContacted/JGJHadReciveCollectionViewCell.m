...
- (void)awakeFromNib {
   ...
   _leftlable = [[UILabel alloc]initWithFrame:CGRectMake(7, CGRectGetHeight(_hadRecive.frame)-2, 38, 2)];
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(dontReply:)];
...
-(void)setNumpepole:(NSString *)numpepole
{
    ...
    if (_taskType) {
        [_DontReply setTitle:@"参与者" forState:UIControlStateNormal];
        if (![numpepole isEqualToString:@"0"]) {
            _numLable.text = [NSString stringWithFormat:@"(%@)",numpepole];
        }else{
            _numLable.text = @"";
        }
        _DontReply.selected = NO;
        ...
        //此处是为了改变后面的颜色
      [_DontReply setTitle:@"未反馈" forState:UIControlStateNormal];
        if (![numpepole isEqualToString:@"0"]) {
        _numLable.text = [NSString stringWithFormat:@"(%@)",numpepole];
        }else{
        _numLable.text = @"";
