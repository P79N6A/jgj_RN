...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"chatlist_more_icon"] forState:UIControlStateNormal];
...
- (void)loadGroupDataFromDB {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.groupDBArr = [[NSMutableArray alloc] initWithArray:[JGJChatMsgDBManger getAllGroupListModel]];
        
        BOOL isGetList = [TYUserDefaults boolForKey:JLGIsRequestChatList];
        if (!isGetList && self.groupDBArr.count == 2) {
            
            [TYUserDefaults setBool:YES forKey:JLGIsRequestChatList];
...
- (void)handleDelButtonPressedWithCell:(JGJContactedListCell *)cell ButtonType:(JGJContactedListButtonType)buttonType {
    ...
    FDAlertView *alert = [[FDAlertView alloc] initWithTitle:nil icon:nil message:@"移除后，将清空该聊天的消息记录" delegate:self buttonTitles:@"取消",@"移除", nil];
    ...
    if (buttonIndex == 1) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (is_delete_success) {
                ...
                [TYShowMessage showSuccess:@"移除消息成功！"];
...
//        [self loadGetTemporaryFriendList]; //根据推送获取是否有新申请人员
...
- (void)goHomeButtonPressedWithProListModel:(JGJChatGroupListModel *)proListModel {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        BOOL needHttp = NO;
        if ([proListModel.class_type isEqualToString:@"team"] || [proListModel.class_type isEqualToString:@"group"]) {
            
            needHttp = YES;
        }
        [JGJChatGetOffLineMsgInfo http_gotoTheGroupHomeVCWithGroup_id:proListModel.group_id class_type:proListModel.class_type isNeedChangToHomeVC:YES isNeedHttpRequest:needHttp success:^(BOOL isSuccess) {
