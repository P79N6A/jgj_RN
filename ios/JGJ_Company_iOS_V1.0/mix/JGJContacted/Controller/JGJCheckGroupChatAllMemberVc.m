...
@property (nonatomic, strong) JGJTeamGroupInfoDetailRequest *sourceInfoDetailRequest; //来源人请求详情页
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //管理员创建者有添加和删除
    cell.memberFlagType = (self.teamInfo.is_admin || [self.teamInfo.is_creater boolValue] || self.workProListModel.teamMangerVcType == JGJProMangerType) ? ShowAddAndRemoveTeamMemberFlagType :  ShowAddTeamMemberFlagType;
    ...
    if (self.memberModels.count > 0) {
        ...
        cell.teamMemberModel = self.memberModels[indexPath.row];
    }
...
- (void)handleUnRegisterMemberCellWithTeamModel:(JGJTeamMemberCommonModel *)commonModel {
    ...
    if ([commonModel.teamModelModel.is_active isEqualToString:@"0"] && self.commonModel.memberType != JGJProSourceMemberType) {
        ...
        alertView.confirmButtonBlock = ^{
            ...
            NSString *url =[NSString stringWithFormat:@"%@page/open-invite.html?uid=%@&plat=manage", JGJWebDiscoverURL,uid];;
            
            NSString *title = @"我在用施工管理神器：吉工宝APP";
            
            NSString *desc = @"项目质量、安全、设备管理更科学更高效，还有大量免费的专项施工资料！";
    ...
    if (self.commonModel.memberType == JGJProSourceMemberType) {
        ...
        [self clickedSourceMember:commonModel];
...
- (void)handleAddMember {
    ...
    if (self.allMemberVcType == CheckAllMemberVcGroupMangerType) {
        ...
        addMemberVc.teamInfo = self.teamInfo;
...
//    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"is_creater != %@ AND uid != %@ AND isMangerModel != %@ AND is_source_member = %@ AND is_agency != %@", @"1", myUid, @(YES), @(NO), @(YES)];
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"is_creater != %@ AND is_admin != %@ AND uid != %@ AND isMangerModel != %@ AND is_source_member = %@", @"1", @(YES), myUid, @(YES), @(NO)];
...
- (void)addMemberSelTypeVcCommonModel:(JGJTeamMemberCommonModel *)commonModel {
    ...
    selTypeVc.successBlock = ^(NSDictionary *response) {
    ...
    //班组项目组添加
    if ([self.teamInfo.class_type isEqualToString:@"group"]) {
        
        selTypeVc.contactedAddressBookVcType = JGJGroupMangerAddMembersVcType;
        
    }else if ([self.teamInfo.class_type isEqualToString:@"team"]) {
...
- (NSMutableArray *)accordTypeGetMangerModels:(JGJTeamInfoModel *)teamInfo {
    ...
    if (teamInfo.is_admin || [teamInfo.creater_uid isEqualToString:myUid]) {
