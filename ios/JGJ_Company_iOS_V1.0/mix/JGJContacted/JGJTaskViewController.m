...
-(float)RowWidth:(NSString *)Str
{
    ...
    paraStyle.headIndent = 0;
    paraStyle.tailIndent = 0;
    NSDictionary *dic = @{NSFontAttributeName:[UIFont systemFontOfSize:13.1], NSParagraphStyleAttributeName:paraStyle, NSKernAttributeName:@0.0f
                          };
...
-(float)RowNoticeHeight:(NSString *)Str otherStr:(NSString *)otherStr
{
    if ([NSString isEmpty:Str]) {
        Str = @"";
    }
    ...
    CGSize size = [Str boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-40 - 16 - [self RowWidth:otherStr], 2000) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
    return size.height + 1;
...
- (void)setIndexProListModel:(JGJChatMsgListModel *)proListModel {
    ...
    [JGJChatGetOffLineMsgInfo http_gotoTheGroupHomeVCWithGroup_id:self.workProListModel.group_id?:@"" class_type:self.workProListModel.class_type?:@"" isNeedChangToHomeVC:YES isNeedHttpRequest:YES success:^(BOOL isSuccess) {
...
- (void)sendText:(JGJChatInputView *)chatInputView text:(NSString *)text {
    ...
    [self GetNetDatamasgID:self.jgjChatListModel.msg_id Action:@"replyMessage" AndReplyext:text];
