...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *activeTrail;
...
- (void)setGroupChatMemberModel:(JGJSynBillingModel *)groupChatMemberModel {
    ...
    self.telphone.text = _groupChatMemberModel.telphone;
    ...
    self.multiSelectedButon.selected = _groupChatMemberModel.isSelected;
    //是否是平台用户标记
    ...
    if ([_groupChatMemberModel.is_active isEqualToString:@"0"]) {
...
//        [self.activeButton setTitle:@"已添加" forState:UIControlStateNormal];
...
- (void)setIsMoveActiveButton:(BOOL)isMoveActiveButton {
    ...
    self.activeTrail.constant = trailing;
