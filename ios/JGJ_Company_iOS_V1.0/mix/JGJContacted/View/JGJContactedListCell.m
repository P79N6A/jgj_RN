...
- (void)handleProNameWithGroupListModel:(JGJChatGroupListModel *)groupModel {
    ...
    if ([groupModel.class_type isEqualToString:@"work"]) {
        ...
        if ([NSString isEmpty:groupModel.title]) {
    ...
    if (unread_msg_count > 9) {
        ...
        self.unReadMsgLabel.frame = CGRectMake(41 + 5, CGRectGetMinY(self.avatarGroupImageView.frame) - 9, 28, 18);
    }else {
        
        self.unReadMsgLabel.frame = CGRectMake(41 + 10, CGRectGetMinY(self.avatarGroupImageView.frame) - 9, 18, 18);
    }
...
- (UILabel *)unReadMsgLabel {
    ...
    if (!_unReadMsgLabel) {
        ...
        _unReadMsgLabel = [[UILabel alloc] initWithFrame:CGRectMake(41 + 10, CGRectGetMinY(self.avatarGroupImageView.frame) - 9, 18, 18)];
...
- (NSString *)dealMessageContentWithGroupLisyModel:(JGJChatGroupListModel *)model {
    ...
    if ([model.class_type isEqualToString:@"activity"]) {
        ...
        }else if ([model.last_msg_type isEqualToString:@"memberjoin"] || [model.last_msg_type isEqualToString:@"delmember"] || [model.last_msg_type isEqualToString:@"upgradegroupchat"]) {
