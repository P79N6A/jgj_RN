...
-(UICollectionView *)peopleCollectionview
{
    if (!_peopleCollectionview) {
        ...
        _peopleCollectionview =[[UICollectionView alloc]initWithFrame:CGRectMake(0, [JGJGetViewFrame GetMaxY:self.NewsLable]-40,[UIScreen mainScreen].bounds.size.width, [self RowNumber]*100) collectionViewLayout:layout];
...
-(UILabel *)DepartLable
{
    if (!_DepartLable) {
        ...
        CGRect rect = CGRectMake(0, [JGJGetViewFrame GetMaxY:self.FromLable]+departHeight, MainScreen.width, 5);
        _DepartLable.textAlignment = rightalign;
        _DepartLable.backgroundColor = [UIColor grayColor];
...
-(void)hadReciveView
{
    ...
    [_bottomview setFrame:CGRectMake(rect.origin.x, [JGJGetViewFrame GetMaxY:self.DepartLable], rect.size.width, [self HeadRowNumber]*50+35)];
...
{
    if (collectionView.tag == 10) {
        return UIEdgeInsetsMake(18.0f, 10.0f, 18.0f, 10.0f);
