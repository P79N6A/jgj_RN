...
- (void)textDidChange
{
    ...
    /*
     ...
     // 最大高度，可以滚动
     self.scrollEnabled = self.canScorll?:(height > _maxTextH && _maxTextH > 0);
     
     _textH = height;
     
     if (_ty_textHeightChangeBlock && self.scrollEnabled == NO) {
     _ty_textHeightChangeBlock(self.text,_textH);
     [self.superview layoutIfNeeded];
...
- (BOOL)textIsReturn:(UITextView *)textView
{
    ...
    //        NSString *text = [textView.text substringFromIndex:textView.text.length - 1];
    //        if ([text isEqualToString:@"\n"]) {
    //            return YES;
    //        }
    //    }
