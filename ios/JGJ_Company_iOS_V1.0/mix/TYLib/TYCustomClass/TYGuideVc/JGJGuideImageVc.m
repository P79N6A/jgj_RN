...
- (void)handleCheckRecordButtonPressed:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(guideImageVcWithguideImageVc:didSelectedFooterView:)]) {
        ...
        JGJWebAllSubViewController *recordWebViewVc = [[JGJWebAllSubViewController alloc] initWithWebType:JGJWebTypeInnerURLType URL:StatisticsDemoWebURL];
        recordWebViewVc.title = @"示例报表";
        
//        __weak typeof(self) weakSelf = self;
//        recordWebViewVc.skipToNextVc = ^(UIViewController *vc){
//
//            weakSelf.navigationController.navigationBar.hidden = NO;
//        };
        [self.navigationController pushViewController:recordWebViewVc animated:YES];
    }
}
