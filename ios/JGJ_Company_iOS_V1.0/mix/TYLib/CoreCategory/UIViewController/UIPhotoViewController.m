...
- (void)setImage:(NSDictionary *)info ByPicker:(UIImagePickerController *)picker{
    ...
    if (!image || ![image isKindOfClass:[UIImage class]]) {
        ...
        if (image.size.width > image.size.height) {
            ...
            ration = image.size.width / image.size.height;
            
            image = [JGJImage imageScale:image withSize:CGSizeMake((int)(maxW * ration), maxW)];
            
        }else {
            
            ration = image.size.height / image.size.width;
            
            image = [JGJImage imageScale:image withSize:CGSizeMake(maxW, (int)(maxW * ration))];
