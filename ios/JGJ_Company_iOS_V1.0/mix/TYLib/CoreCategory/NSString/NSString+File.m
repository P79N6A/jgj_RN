...
-(NSString *)createSubFolder:(NSString *)subFolder{
    NSString *subFolderPath=[NSString stringWithFormat:@"%@/%@",[NSString documentFolder],subFolder];
    ...
    if ( !(isDir == YES && existed == YES) )
    {
        ...
        BOOL isSucced  = [fileManager createDirectoryAtPath:subFolderPath withIntermediateDirectories:YES attributes:nil error:nil];

        TYLog(@"创建文件 %@",isSucced?@"成功":@"失败");
    }
