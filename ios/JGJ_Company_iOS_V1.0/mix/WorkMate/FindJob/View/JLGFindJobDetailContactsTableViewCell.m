...
//    self.yourFirendsLabel.text = [NSString stringWithFormat:@"你有%@个朋友认识班组长",@(friendsInfoArray.count)];
...
- (void)setFirendsLabelText:(NSInteger )friednsCount{
    ...
    if (!self.frontString || !self.behindString) {
        selectedStr = [NSString stringWithFormat:@"你有%@个朋友认识班组长",selectedNumStr];
    }else{
