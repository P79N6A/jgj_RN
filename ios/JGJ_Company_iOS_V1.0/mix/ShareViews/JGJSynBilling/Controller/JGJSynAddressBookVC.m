...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.addSynModelBlock = ^(JGJSynBillingModel *addressBookModel){
        ...
        if (_addressBookAddButtonType != AddressBookAddWorkerButton) {
            ...
            if ([NSString containsEmojiStr:addressBookModel.name]) {
                ...
                namePopView.tel.text = addressBookModel.telph;
                ...
                namePopView.confirmBlock = ^(JGJInputNamePopView *popView) {
                    ...
                    [weakSelf upLoadAddressBookWithAddressBookModel:addressBookModel];
...
//- (IBAction)autoOpenAddressBookButtonPressed:(UIButton *)sender {
//    
//    [self cfreRreshStatusViewButtonPressedWithcfreRreshStatusView:nil];//手动打开通信录权限
//}
...
- (void)SynBillAddContactsHubSaveSuccess:(JGJSynBillAddContactsHUBView *)contactsView{
    ...
    if ([telePhone isEqualToString:contactsView.phoneNumTF.text]) {
        [TYShowMessage showError:@"操作失败,无法对自己同步项目!"];
        return;
    }
