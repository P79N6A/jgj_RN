...
@property (weak, nonatomic) IBOutlet UILabel *synProCounLable;
...
- (void)awakeFromNib {
    ...
    self.synProCounLable.textColor = AppFont666666Color;
    ...
    [self.synProCounLable.layer setLayerCornerRadius:2.5];
}
...
- (void)setSynBillingModel:(JGJSynBillingModel *)synBillingModel {
    ...
    self.synProCounLable.text = [NSString stringWithFormat:@"%@个", synBillingModel.sync_pro_num];
    [self.synProCounLable markText:@"个" withColor:AppFont999999Color];
    self.containDetailView.hidden = self.synBillingCommonModel.isWageBillingSyn;
//    同步账单管理 ,项目进入同步账单管理
