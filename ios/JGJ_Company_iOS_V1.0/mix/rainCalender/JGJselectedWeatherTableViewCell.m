...
-(void)clickTopbutton:(NSString *)buttonTitle
{
    if ([buttonTitle isEqualToString:@"确定"] || [buttonTitle isEqualToString:@"关闭"]) {
        if ([buttonTitle isEqualToString:@"关闭"]) {
            
#pragma mark - 后期添加 用于不选了就隐藏选中提示框
...
-(UIImageView *)centerPlaceView
{
    if (!_centerPlaceView) {
        ...
        _centerPlaceView.backgroundColor = [UIColor redColor];
        
        _centerPlaceView.layer.masksToBounds = YES;
        _centerPlaceView.layer.cornerRadius = CGRectGetWidth(_centerPlaceView.frame)/2;
//        _centerPlaceView.layer.borderWidth = 6;
//        _centerPlaceView.layer.borderColor = AppFontf1f1f1Color.CGColor;
//        _centerPlaceView.layer.opaque = .18;
...
//        layer.strokeColor   = AppFontd7252cColor.CGColor;   // 边缘线的颜色
//        
//        layer.fillColor     = AppFontf1f1f1Color.CGColor;
//        // 闭环填充的颜色
...
//        
//        
////        layer.shadowColor=AppFontEB4E4EColor.CGColor;
...
- (UITableView *)tableview
{
    if (!_tableview) {
        ...
        _tableview.backgroundColor = AppFontfafafaColor;
