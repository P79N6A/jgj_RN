...
- (void)layoutPhotoLoadOriginButton
{
    ...
    if ([_photo respondsToSelector:@selector(loadOriginButton)])
    {
        ...
        if (_loadOriginButton)
        {
            ...
            [_loadOriginButton setFrame:CGRectMake((TYGetUIScreenWidth - 148)/2, TYGetUIScreenHeight-42, 148, 32)];
...
- (void)setMaxMinZoomScalesForCurrentBounds {
    ...
    if (imageSize.height > imageSize.width) {//超长图特殊处理
        if (imageSize.height/imageSize.width > [[UIScreen mainScreen] bounds].size.height/TYGetUIScreenWidth) {
            minScale = xScale;
            maxScale = xScale*2;
        }
