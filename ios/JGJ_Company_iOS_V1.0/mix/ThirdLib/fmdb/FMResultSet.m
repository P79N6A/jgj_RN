...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        TYLog(@"Warning: There seem to be no columns in this set.");
    }
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        TYLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        TYLog(@"Database busy");
        if (outErr) {
        ...
        TYLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        // wtf?
        TYLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    TYLog(@"Warning: I could not find the column named '%@'.", columnName);
