...
+ (NSString *)monthFromCppToMineWithYear:(NSUInteger)_year month:(NSUInteger)_month {
    ...
    if (run==0) {
        return [NSString stringWithFormat:@"%@-%ld", CHINESE_MONTH_NORMAL, (unsigned long)_month];
    } else {
        if (_month==run+1) {
            return [NSString stringWithFormat:@"%@-%lu", CHINESE_MONTH_LUNAR, _month-1];
        } else if(_month<=run) {
            return [NSString stringWithFormat:@"%@-%ld", CHINESE_MONTH_NORMAL, (unsigned long)_month];
        } else {
            return [NSString stringWithFormat:@"%@-%lu", CHINESE_MONTH_NORMAL, _month-1];
        }
    }
}
