...
static float const kPopoverViewCellHeight = 61.f; ///< cell指定高度
...
- (void)layoutSubviews {
    ...
    CGFloat height = self.actions.count + kPopoverViewCellHeight + kPopoverViewArrowHeight + 100;
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    float arrowCornerRadius = 1.3;
    float arrowBottomCornerRadius = 0.5;
    ...
    //yj3.5.2添加
    CGFloat height = self.actions.count * kPopoverViewCellHeight + kPopoverViewArrowHeight;
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<PopoverAction *> *)actions {
    ...
    // 弹窗箭头指向的点
    CGFloat pointViewoffsetX = 15.0; //2.1.0-yj添加
    CGFloat pointViewoffsetY = 10.0; //2.1.0-yj添加
    CGPoint toPoint = CGPointMake(CGRectGetMidX(pointViewRect) + pointViewoffsetX, 0);
    // 弹窗在 pointView 顶部
    if (pointViewUpLength > pointViewDownLength) {
        toPoint.y = pointViewUpLength - pointViewoffsetY;
    }
    // 弹窗在 pointView 底部
    else {
        toPoint.y = CGRectGetMaxY(pointViewRect) + pointViewoffsetY;
    }
