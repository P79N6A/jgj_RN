...
@property (nonatomic, strong) NSArray *unCloseList;

@property (nonatomic, strong) NSArray *closeList;
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:rightItemBtn];
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"删除项目说明" style:UIBarButtonItemStylePlain target:self action:@selector(rightItemPressed:)];
    
    TYWeakSelf(self);
    
    self.buttonView.handleCusBottomButtonViewBlock = ^(JGJCusBottomButtonView *buttonView) {
        
        [weakself handleCreatGroup];
...
- (void)rightItemPressed:(UIBarButtonItem *)item {
    ...
    NSString *webUrl = [NSString stringWithFormat:@"%@help/hpDetail?id=%@", JGJWebDiscoverURL,@"240"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.unCloseList.count > 0 && indexPath.section == 0) {
        ...
        closeProListCell.lineView.hidden = self.closeList.count -1 == indexPath.row;
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    ...
    return section == 0 ? 20 : CGFLOAT_MIN;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    UIView *footerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TYGetUIScreenWidth, 20)];
...
- (void)checkProHeaderView:(JGJCheckProHeaderView *)headerView {
    ...
    TYLog(@"headerView.tag ===== %@", @(headerView.tag));
...
- (JGJCusBottomButtonView *)buttonView {
    ...
    if (!_buttonView) {
        ...
        [_buttonView.actionButton setTitle:@"新建项目" forState:UIControlStateNormal];
