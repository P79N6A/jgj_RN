...
@property (strong, nonatomic) UIView *containSourceView;
...
- (void)viewDidLoad {
    ...
    if (self.commonModel.teamControllerType != JGJCreatTeamControllerType) {
...
- (void)unfoldAddressBook:(NSArray *)addressBookModels {
    ...
    .count > 0  || self.currentTeamMembers.count > 0;
...
- (void)getContactsFirstletter:(NSMutableArray *)contacts {
    ...
    if (self.sortContactsModel.sortContacts.count == 0 || !self.sortContactsModel) {
...
- (void)showSynBillingSelectedCount:(NSMutableArray *)synBillings {
    ...
    BOOL isShowHeader = self.commonModel.teamControllerType == JGJTeamMangerControllerType;
...
- (IBAction)confirmButtonPressed:(UIButton *)sender {
    ...
    for (SortFindResultModel *sortFindResultModel in self.sortContacts) {
        for (JGJSynBillingModel *teamMemberModel in sortFindResultModel.findResult) {
            if (teamMemberModel.isSelected) {
                [self.selectedSynBillingModels addObject:teamMemberModel];
            }
        }
...
//点击加号判断是否弹框
    if (self.groupMemberMangeType != JGJGroupMemberMangeRemoveMemberType) {
        ...
        if ([self handleMaxSelMemberWithSelMembers:self.selectedSynBillingModels]) {
    ...
    if (self.groupMemberMangeType == JGJGroupMemberMangePushNotifyType) {
        ...
        desModel.popDetail = @"发送消息给对对方，通知他将项目数据同步，对方同意后，会自动将他加入到项目中";
        desModel.leftTilte = @"取消";
        desModel.rightTilte = @"立即发送";
...
- (void)JGJUnhandleSourceListVcConfirmButtonPressed:(JGJUnhandleSourceListVC *)sourceListVC {
    ...
    JGJCreatProAddDataSourceVC *addDataSourceVC = nil;
    if ([self.delegate isKindOfClass:[JGJCreatProAddDataSourceVC class]]) {
        addDataSourceVC = (JGJCreatProAddDataSourceVC *)self.delegate;
    }
    addDataSourceVC.sourceSynProFirstModel = sourceListVC.synProFirstModel;
    addDataSourceVC.selelctedSourcePros = selecteSourceMember;
    if ([self.delegate respondsToSelector:@selector(handleJGJGroupMemberSelectedTeamMembers:groupMemberMangeType:)]) {
...
#pragma mark - 添加单个成员
- (IBAction)addTeamMemberModelButtonPressed:(UIBarButtonItem *)sender {
...
- (void)SynBillAddContactsHubSaveSuccess:(JGJSynBillAddContactsHUBView *)contactsView {
    ...
    NSString *myPhone = [TYUserDefaults objectForKey:JLGPhone];
    if ([teamModel.telephone isEqualToString:myPhone] && !teamModel.isAddedSyn) {
    ...
    JGJSynBillingModel *addedMemberModel = [self.currentTeamMembers filteredArrayUsingPredicate:predicate].lastObject;
    if (addedMemberModel.isAddedSyn) {
        [TYShowMessage showPlaint:@"已添加!"];
        return;
    }
    [self handleAddIndividualTeamMember:teamModel];
}
...
- (void)handleAddIndividualTeamMember:(JGJSynBillingModel *)teamModel {
    ...
    BOOL isAddContact = [self.selectedSynBillingModels filteredArrayUsingPredicate:predicate].count == 0;
    if (isAddContact && !teamModel.isAddedSyn && !teamModel.isSingleChatModel) { //未选中或者已经是成员
        [self.selectedSynBillingModels addObject:teamModel]; //添加为已选中数据
    }
...
//    }];
    ...
    if (!isContainModel) { //注意此处判断是否存在原数据组，若已全移除，数据返回时要重新创建
        ...
        //    判断是否有相同的人添加
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"telph contains %@", synBillingModel.telephone];
        NSArray *addedContacts = [sortContacts filteredArrayUsingPredicate:predicate];
        if (addedContacts.count > 0) { //当前人员存在并且标记为已添加
            ...
            if (containContactModel.findResult.count > 0) {
...
- (void)firstOpenAdddressbook {
    ...
    if (self.sortContactsModel.sortContacts.count == 0) { //未缓存成功重新缓存
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [JGJAddressBookTool archiveSortContactsModel:self.sortContactsModel contactsPath:JGJAddressBookToolPath];// 联系人存储路径
...
- (void)handleCurrentTeamMember:(JGJAddressBookSortContactsModel *)sortContactsModel {
    ...
    if (self.commonModel.teamControllerType == JGJTeamMangerControllerType || [self isLoadSingleChatList]) {
//        [self loadSingleChatList]; //加载通信录单聊列表
        
        //判断通信录是否打开
        [self unfoldAddressBook:nil];
        
        //当前班组人员状态去对比通信录人员状态 2.1.0 这里需要重写写得不好
...
#pragma mark - 是否加载单聊人员1.1.0审核关于添加人员失败解决方案
- (BOOL)isLoadSingleChatList {
    return YES;
}
...
- (void)commonSet {
    ...
    self.containSourceNumView.backgroundColor = AppFontfdf1e0Color;
...
//    NSDictionary *parameters = [self.singleListRequest mj_keyValues];
//    __weak typeof(self) weakSelf = self;
//    [JGJSocketRequest WebSocketWithParameters:parameters success:^(id responseObject) {
//        JGJSingleChatModel *singleChatModel = [JGJSingleChatModel mj_objectWithKeyValues:responseObject];
//        weakSelf.singleChatMembers = singleChatModel.member_list;
...
//判断有没打开通讯录
//     [self unfoldAddressBook:_singleChatMembers];
//    
//    if (_singleChatMembers.count > 0) { //通信录单聊过的数据
//        for (JGJSynBillingModel *teamMember in _singleChatMembers) {
//            teamMember.isAddedSyn = NO; //单聊的数据初始状态没有添加
//            teamMember.isSingleChatModel = YES; //单聊模型初始化不添加到已选中
//            [self handleAddIndividualTeamMember:teamMember];
//        }
//    }
