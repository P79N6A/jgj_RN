...
- (void)handleUploadTeamMembers:(NSMutableArray *)groupMembersInfos groupMemberMangeType:(JGJGroupMemberMangeType)groupMemberMangeType {
    ...
    if (self.commonModel.memberType == JGJProMemberType && groupMemberMangeType != JGJGroupMemberMangePushNotifyType) {
...
- (NSMutableArray *)dataSource {
    ...
    if (!_dataSource) {
        ...
        NSArray *titles = @[@"从手机通讯录添加", @"手动添加", @"从项目选择添加",@"邀他扫描吉工宝二维码加入"];
        NSArray *icons = @[@"from_addressbook_icon", @"from_tel_icon",@"from_pro_add_icon", @"sweep_Qrcode_join_team"];
...
- (JGJImageModelView *)imageModelView {
    ...
    if (!_imageModelView) {
        ...
        _imageModelView.backgroundColor = AppFontffffffColor;
    }
