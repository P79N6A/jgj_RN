...
//                height = [self calculateCollectiveViewHeight:self.teamMemberModels memberFlagType:ShowAddAndRemoveTeamMemberFlagType] - HeaderHegiht / 2.0;
        ...
        case 1: {
            ...
            if (indexPath.row == 0) {
                ...
                //                height = [self calculateCollectiveViewHeight:commonModel.teamMemberModels memberFlagType:DefaultTeamMemberFlagType];
...
//    editTeamNameVc.defaultProName = self.teamInfo.nickname;
...
//
//        self.checkAllMembers = _teamInfo.source_members;
//    }
//
...
- (void)handleYZGOnlyAddProjectViewControllerEditName:(NSString *)editName editType:(EditNameType )editType{
    ...
    if (editType == EditMineNameType) {
        ...
        self.modifyTeamInfoRequestModel.group_name = nil;
        [self modifyTeamInfo:self.modifyTeamInfoRequestModel];
...
- (UITableViewCell *)handleRegisterProInfoTableView:(UITableView *)tableView indexpath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        JGJCreatTeamModel *creatTeamModel = self.creatTeamModels[indexPath.row];
        
        creatTeamModel.isHiddenArrow = NO;
        
        if (indexPath.section == 2 && indexPath.row == 2) {
            
            teamCell.title.hidden = YES;
            
            creatTeamModel.isHiddenArrow = YES;
...
- (void)handleShutTeam {
    ...
    alertView.onOkBlock = ^{
        ...
        [JLGHttpRequest_AFN PostWithNapi:JGJCloseGroupURL parameters:parameters success:^(id responseObject) {
            ...
            UIViewController *homeVc = weakSelf.navigationController.viewControllers[0];
...
//    CustomAlertView *alertView = [CustomAlertView showWithMessage:@"确认退出项目组吗？" leftButtonTitle:@"取消" midButtonTitle:nil rightButtonTitle:@"确定"];
