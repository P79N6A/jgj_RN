...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *QRCodeImageCenterY;
...
//    [self.QRCodeImage addGestureRecognizer:longPresssRecognizer];
    ...
    [self.middleView.layer setLayerCornerRadius:JGJCornerRadius / 2.0];
    
    self.contentDetailLable.textColor = AppFont666666Color;
}
...
//    
//    JGJHeadView *head = [[JGJHeadView alloc]initWithFrame:CGRectMake(0, 0, 51, 51) withframe:self.proListModel.members_head_pic];
//    [self.avatarImage addSubview:head];
    
    [self.avatarImage getRectImgView:self.proListModel.members_head_pic];
    NSString *proName = @"";
    if (![NSString isEmpty:self.proListModel.pro_name] && [self.proListModel.class_type isEqualToString:@"group"] && ![self.proListModel.pro_name isEqualToString:self.proListModel.group_name]) {
        proName = [NSString stringWithFormat:@"%@-",self.proListModel.pro_name];
    }
    self.topTitleLabel.text = [NSString stringWithFormat:@"%@%@", proName,self.proListModel.group_name];
    ...
    if ([self.proListModel.class_type isEqualToString:@"team"]) {
        ...
        self.contentDetailLable.text = @"使用吉工宝APP扫描以上二维码，\n即可加入此项目组";
        lineText = @"即可加入此项目组";
...
- (void)createQRCodeImageWithUrl:(NSString *)codeUrl{
    ...
    CGFloat myQRCodeImageRation = 0.3;
    
//    CGFloat QRCodeImageW = myQRCodeImageRation * TYGetUIScreenWidth;
    
    if (![self.proListModel.class_type isEqualToString:@"addFriend"]) {

        self.QRCodeImageW.constant = myQRCodeImageRation * TYGetUIScreenWidth;
...
//        self.QRCodeImageCenterY.constant -= 20;
