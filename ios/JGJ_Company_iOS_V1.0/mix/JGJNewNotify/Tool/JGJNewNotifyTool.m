...
+ (NSArray *)allReadedNofies {
    ...
    while (set.next) {
        NSArray *classtypes = @[@"syncProject",@"newBilling", @"syncNoticeTarget"];
    ...
    NSString *updateSql = [NSString stringWithFormat:@"update t_collect_notify set isReaded=1 where userUid=%@ and class_type!='syncProject'  and  class_type!='newBilling' and  class_type!='syncNoticeTarget'", UserUid];
...
+ (BOOL)addCollectNotifies:(JGJNewNotifyModel *)notifyModel
{
    ...
    BOOL isInsertSuccess = [_db executeUpdateWithFormat:@"INSERT INTO t_collect_notify(notify, class_type, notice_id, date,isReaded, userUid) VALUES(%@, %@, %@, %@, %@, %@);", data, notifyModel.class_type,notifyModel.notice_id, notifyModel.date, @(notifyModel.isReaded), UserUid];
   return  isInsertSuccess;
}

+ (BOOL)removeCollectNotify:(JGJNewNotifyModel *)notifyModel
{
    BOOL isRemoveSuccess = [_db executeUpdateWithFormat:@"DELETE FROM t_collect_notify WHERE notice_id = %@ AND userUid = %@;", notifyModel.notice_id, UserUid];
...
- (void)handleSuccessGetNoticeList:(id)responseObject {
    ...
    if (_notifyTool.notifyToolBlock) {
