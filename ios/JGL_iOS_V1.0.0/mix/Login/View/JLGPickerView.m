...
- (void)showPickerByrow:(NSIndexPath *)row dataArray:(NSArray *)dataArray title:(NSString *)title isMulti:(BOOL)isMulti andArr:(NSMutableArray *)arr
{
    ...
    [self.selectedComponentsArray enumerateObjectsUsingBlock:^(NSNumber *obj, NSUInteger idx, BOOL *stop) {
        ...
        statusLable.textColor = AppFontEB4E4EColor;
    ...
    if (arr.count) {
        ...
        for (int index = 0; index < self.dataArray.count; index ++) {
            ...
            if ([[NSString stringWithFormat:@"%@",self.dataArray[index][@"id"]] isEqualToString: [NSString stringWithFormat:@"%@",arr[0][@"id"]]]) {
            
                _selectIndex = index;
                [weakSelf.selectPicker selectRow:index inComponent:0 animated:NO];
            }
        }
        
        if (!arr.firstObject[@"id"] || ![arr.firstObject[@"name"] length]) {
           
            _selectIndex = 0;
            [weakSelf.selectPicker selectRow:0 inComponent:0 animated:NO];
        }
    }
     ...
     } completion:^(BOOL finished)
     {
         if (finished)
         {
             if (weakSelf.selectedComponentsArray.count > 0) {
                 ...
                 if (self.enterType == EnterPickerViewTypeLeftMenu) {
                     
                     [weakSelf setFinishArrayDatainComponent:weakSelf.indexPath.section row:weakSelf.indexPath.row];
                     
                 }else{
//                     if (arr) {
//                         self.finishArray = arr;
//                     }
...
//    1.4.5新增编辑按钮
    if (sender.tag == 100) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(JGJPickViewEditButtonPressed:)]) {
