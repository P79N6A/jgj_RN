...
- (IBAction)goToAgreementBtnClick:(id)sender {
    JGJWebAllSubViewController *webViewController = [[JGJWebAllSubViewController alloc] initWithWebType:JGJWebTypeAgreement];//进入网页界面
...
- (IBAction)nextStepBtnClick:(UIButton *)sender {
    if (self.phoneTF.text.length == 11)
    {
        ...
        [parametersDic setObject:(JLGisMateBool?@1:@2) forKey:@"role"];
        ...
        [JLGHttpRequest_AFN PostWithApi:@"jlsignup/register" parameters:parametersDic success:^(id responseObject) {
            ...
            UIViewController *authorizationVc = [[UIStoryboard storyboardWithName:@"Login" bundle:nil] instantiateViewControllerWithIdentifier:@"authorizationRegister"];
            [weakSelf.navigationController pushViewController:authorizationVc animated:YES];
