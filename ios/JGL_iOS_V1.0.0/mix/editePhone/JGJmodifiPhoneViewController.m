...
//    _phoneTextfiled.borderStyle = uikeyboard
    [super viewDidLoad];
    [self loadviewxib];
    self.title = @"修改手机号码";
...
-(void)loadviewxib{
    ...
    if (_isFineAccountInto) {
        
        _notUsebutton.hidden = YES;
        [_nextstpbutton setTitle:@"确认更换" forState:(UIControlStateNormal)];
        _phoneTextfiled.layer.borderWidth = 1;
        _phoneTextfiled.layer.borderColor = AppFontdbdbdbColor.CGColor;
        _phoneTextfiled.layer.cornerRadius = 5;
        _phoneTextfiled.textAlignment = NSTextAlignmentCenter;
    }
}
-(void)taplable
{
...
- (void)getCheckNum {
    if (_isFineAccountInto) {
        ...
        [JLGHttpRequest_AFN PostWithApi:@"v2/signup/getVcodeToFindAccount" parameters:@{@"telephone":newPhoneNumber} success:^(id responseObject) {
...
- (void)setIsFindAccoutInWithToken:(BOOL)isFindAccountIn token:(NSString *)token {
    ...
    _isFineAccountInto = isFindAccountIn;
    _token = token;
...
- (void)modifyPhoneNumber {
    ...
    if (_isFineAccountInto) {
        ...
        [paramDic setObject:modifyNum?:@"" forKey:@"vcode"];
        [paramDic setObject:_token?:@"" forKey:@"token"];
        [paramDic setObject:_phoneTextfiled.text?:@"" forKey:@"telephone"];
        
        [JLGHttpRequest_AFN PostWithApi:@"v2/signup/changeTelephone" parameters:paramDic success:^(id responseObject) {
            
            if ([responseObject[@"status"] integerValue] == 1) {
                
                for (UIViewController *vc in self.navigationController.viewControllers) {
                    
                    if ([vc isKindOfClass:[JLGLoginViewController class]]) {
                        
                        [self.navigationController popToViewController:vc animated:YES];// 跳回到登陆页面
                    }
                }
