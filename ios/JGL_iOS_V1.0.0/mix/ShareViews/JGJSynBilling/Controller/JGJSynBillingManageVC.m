...
@interface JGJSynBillingManageVC () <UITableViewDelegate, UITableViewDataSource,UITextFieldDelegate, CFRefreshStatusViewDelegate, DSectionIndexViewDataSource,DSectionIndexViewDelegate, YZGOnlyAddProjectViewControllerDelegate>
...
@property (weak, nonatomic) IBOutlet UIView *contentopView;
...
- (void)viewDidLoad {
    ...
    [self.contentopView addGestureRecognizer:tap];
    
    self.contentopView.hidden = YES;
}
...
- (IBAction)confirmSynButtonPressed:(UIButton *)sender {
    ...
    for (JGJSynBillingModel *synBillingModel in self.selectedSynBillingModels) {
        [upLoadProinfoStr appendFormat:@"%@,%@,%@;",synBillingModel.target_uid, @(self.wageMoreDetailInitModel.infoId),self.wageMoreDetailInitModel.name];
    }
//    删除末尾的分号
    ...
    NSString *pid = [NSString stringWithFormat:@"%@", @(self.wageMoreDetailInitModel.infoId)];
...
- (void)loadNetData {
    ...
    if (self.synBillingCommonModel.isWageBillingSyn) {
         NSString *pid = [NSString stringWithFormat:@"%@", @(self.wageMoreDetailInitModel.infoId)];
        parameters = @{@"pid" : pid ?:[NSNull null]
                       }.mutableCopy;
    }
    __weak typeof(self) weakSelf = self;
    ...
    [self.tableView loadWithViewOfStatus:^UIView *(CFRefreshTableView *tableView, ERefreshTableViewStatus status) {
        ...
        self.contentSearchBarViewH.constant = self.tableView.dataArray.count == 0 ? 0 : 48;
        self.contentSearchBarView.hidden = self.tableView.dataArray.count == 0;
        return view;
...
//    [self.tableView headerBegainRefreshing];
    ...
    self.topViewH.constant = self.dataSource.count == 0 ? 0 : 45;
    
    self.contentopView.hidden = self.dataSource.count == 0;
...
- (void)cfreRreshStatusViewButtonPressedWithcfreRreshStatusView:(CFRefreshStatusView *)statusView {
    ...
    if (self.synBillingCommonModel.isWageBillingSyn) {
        ...
        if (!self.iSExistPro) {
            ...
            nextVc = [self handleCreatProAction];
            
        }else {
            
            JGJSynAddressBookVC *synAddressBookVC = [[UIStoryboard storyboardWithName:@"JGJSynBilling" bundle:nil] instantiateViewControllerWithIdentifier:@"JGJSynAddressBookVC"];
            
            nextVc = synAddressBookVC;
        }
    }
...
- (void)synHelpCenter {
    ...
    NSString *url = [NSString stringWithFormat:@"%@help/hpDetail?id=91", JGJWebDiscoverURL];
...
#pragma mark - 创建项目
- (UIViewController *)handleCreatProAction {
