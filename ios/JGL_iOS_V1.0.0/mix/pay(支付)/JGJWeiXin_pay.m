...
- (void)onResp:(BaseResp *)resp {
    if ([resp isKindOfClass:[SendMessageToWXResp class]]) {
        ...
        [self getWeiXinWithAuthResp:authResp];
//        if (_delegate
...
//        }
    ...
    }else if([resp isKindOfClass:[PayResp class]]){ //3.2.0支付修改
        //支付
...
+ (void)creatWeiXinPayOrderForGoods
{
    ...
    NSDictionary *paramDic = @{@"appid":@"wx0d7055be43182b5e",//应用id 是
...
{
//    key为商户平台设置的密钥key
    NSString *appidStr = [[@"appid=" stringByAppendingString:@"wx0d7055be43182b5e"] stringByAppendingString:@"&"];
...
- (void)doAlipayPaypayCode:(NSString *)paycode andmodel:(JGJOrderListModel *)model
{
    ...
    NSString *appScheme = @"AlipaycomPerson";
...
- (void)getWeiXinWithAuthResp:(SendAuthResp *)authResp{
    NSString *url =[NSString stringWithFormat:@"https://api.weixin.qq.com/sns/oauth2/access_token?appid=%@&secret=%@&code=%@&grant_type=authorization_code",@"wx0d7055be43182b5e",@"d4624c36b6795d1d99dcf0547af5443d",authResp.code];
...
//                    [TYShowMessage showSuccess:@"绑定微信账户成功"];
                    
                }else{
//                    [TYShowMessage showSuccess:@"绑定微信账户失败"];
