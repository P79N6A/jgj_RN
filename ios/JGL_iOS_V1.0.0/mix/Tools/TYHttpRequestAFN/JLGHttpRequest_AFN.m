...
@property (nonatomic, assign) CGFloat timeInterval; //时间间隔 NSURLSessionDownloadTask *task
...
//    mutableParameters[@"sign"] = JGJSHA1Sign;
    ...
    [mgr GET:url parameters:[JLGHttpRequest_AFN accordingDicReturnMutableDic:mutableParameters] progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        //打印正确的信息
        [self showSuccessMessageByUrl:url respondse:responseObject parameters:mutableParameters requestOperation:task];
        ...
        //打印并显示错误信息
        [self showFailureMessageByUrl:url parameters:mutableParameters requestOperation:task];
...
+ (NSURLSessionDataTask *)PostWithApiReturnTask:(NSString *)api parameters:(id)parameters success:(void (^)(id responseObject))success failure:(void (^)(NSError *error))failure
{
    ...
    NSURLSessionDataTask *ljTask = [mgr POST:url parameters:[JLGHttpRequest_AFN accordingDicReturnMutableDic:mutableParameters] progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        //        NSLog(@"请求花费时间:%f",[[NSDate date] timeIntervalSince1970] - requestTime);
        NSUInteger state = [responseObject[@"state"] integerValue];
        //打印正确的信息
        [self showSuccessMessageByUrl:url respondse:responseObject parameters:[JLGHttpRequest_AFN accordingDicReturnMutableDic:mutableParameters] requestOperation:task];
...
+ (NSURLSessionDataTask *)PostWithNApiReturnTask:(NSString *)api parameters:(id)parameters success:(void (^)(id responseObject))success failure:(void (^)(NSError *error))failure
{
...
//            failure(nil);
//        }
        if ([responseObject[@"errno"] intValue] == 20027) {
            success(responseObject);
   ...
   __block NSInteger count = [[TYUserDefaults objectForKey:JGJKnowBaseShareCount] integerValue];
    ...
    if (count >= 5) {
        ...
        alertView.onOkBlock = ^{
            ...
            NSString *url = @"http://m.jgjapp.com/";
            
            NSString *title = [NSString stringWithFormat:@"%@.%@", downModel.file_name, downModel.file_type];
            
            [_httpRequest_AFN.knowledgeDaseTool showShareBtnClick:img desc:@"我在吉工家资料库下载了该资料，更有大量专项施工资料任你免费获取！" title:title url:url];
...
+ (NSDictionary *)accordingDicReturnMutableDic:(id)dic
{
    ...
    //时间戳
//    NSString *oldStamp = [TYUserDefaults objectForKey:JLGUserstamp]?:@"0";
...
//    NSString *timeStampStr = [NSString stringWithFormat:@"%ld",stamp];
    ...
    //客户端类型(区分吉工家or吉工宝)
    [SignParmDic setObject:@"person" forKey:@"client_type"];
    //系统类型(区分iOS和android)
