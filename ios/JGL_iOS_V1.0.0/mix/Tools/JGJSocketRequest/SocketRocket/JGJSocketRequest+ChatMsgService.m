...
//用于消息里面添加人时，接收消息
static NSString *friendMessageToSender = @"friendMessageToSender";
...
+(void)insertWebSocketReceiveChatMsg:(id)receive {
    ...
    TYLog(@"WebSocketReceivereceivemsgs = %@",receive);
    ...
    //处理线面四种情况
    if (!([action isEqualToString:sendMessage] || [action isEqualToString:receiveMessage] || [action isEqualToString:recallMessage] || [action isEqualToString:friendMessageToSender])) {
        ...
        if ([action isEqualToString:reddotMessage]) {
            ...
            if ([receive[@"result"][@"msg_type"] isEqualToString:@"reply"]) {
                ...
                if ([receive[@"result"][@"group_id"] isEqualToString:indexModel.group_id] && [receive[@"result"][@"class_type"] isEqualToString:indexModel.class_type]) {
                    ...
                    // 更新聊聊列表对应的工作消息回复数
                    [JGJChatMsgDBManger updateChatGroupListTableTheWork_Replay_countWithCount:indexModel.work_message_num group_id:indexModel.group_id class_type:indexModel.class_type];
                    
                    if (updateSuccess) {
                        
                        [JGJChatGetOffLineMsgInfo refreshIndexTbToHomeVC];
                    }
                }else {// 更新聊聊列表对应的工作消息回复数
                    
                    JGJChatGroupListModel *model = [JGJChatMsgDBManger getChatGroupListModelWithGroup_id:receive[@"result"][@"group_id"] classType:receive[@"result"][@"class_type"]];
                    [JGJChatMsgDBManger updateChatGroupListTableTheWork_Replay_countWithCount:[NSString stringWithFormat:@"%ld",[model.work_message_num integerValue] + 1] group_id:indexModel.group_id class_type:indexModel.class_type];
                }
                
            }else if ([receive[@"result"][@"msg_type"] isEqualToString:@"set_agency"]) {// 代理人信息推送
                
                [JGJChatGetOffLineMsgInfo http_getSingleChatGroupWithGroup_id:receive[@"result"][@"group_id"] class_type:receive[@"result"][@"class_type"] success:^(JGJChatGroupListModel *insertGroupModel) {
...
+(void)handleReceiveGroupMsgs:(NSArray *)msgs action:(NSString *)action {
    ...
    if ([action isEqualToString:receiveMessage]) { //处理receiveMessage
...
+(void)insertDBMsgs:(NSArray *)msgs action:(NSString *)action {
    ...
    for (int i = 0; i < msgArr.count; i ++) {
        ...
        if ([msgModel.msg_type isEqualToString:@"recall"]) {
            ...
            if ([oriRecallMsgModel.msg_sender isEqualToString:myUid]) {

                oriRecallMsgModel.msg_text = @"你 撤回一条消息";

            }else {

                oriRecallMsgModel.msg_text = [NSString stringWithFormat:@"%@ %@", oriRecallMsgModel.user_info.real_name,@"撤回一条消息"];
            }

            oriRecallMsgModel.msg_type = msgModel.msg_type;

            oriRecallMsgModel.sys_msg_type = msgModel.sys_msg_type?:@"system";

            msgModel = oriRecallMsgModel;
            
        }
        
        BOOL is_success = NO;
        
        TYLog(@"message-------%@------%@  %@", msgModel.msg_text, msgModel.msg_type, msgModel.msg_id);
...
+ (BOOL)insertDBMsgWithChatMsgListModel:(JGJChatMsgListModel *)msgModel action:(NSString *)action{
    ...
    BOOL insertS = [JGJChatMsgDBManger insertToChatMsgTableWithJGJChatMsgListModel:msgModel propertyListType:JGJChatMsgDBUpdateRecallPropertyType];
...
+ (void)callBackMsgWithMsgModel:(JGJChatMsgListModel *)msgModel action:(NSString *)action{
    ...
    BOOL is_can_receive = shareSocket.receivedMsgCallback && [self readMsgWithMsgModel:msgModel] && ([action isEqualToString:receiveMessage] || [action isEqualToString:friendMessageToSender]);
