...
static NSString *dealCompleteMsg_callback_key = @"dealCompleteMsg_callback_key";
...
+(void)receiveMsgCallBackGroup:(NSArray *)msgs maxMsgs:(NSArray *)maxMsgs type:(JGJMsgCallBackType)type {
    ...
    NSArray *normalMsgs = [msgs filteredArrayUsingPredicate:normalPredicate];
...
+ (void)dealOffLineNormalMsg:(NSArray *)normalMsgs {
    ...
    if ([JGJSocketRequest shareSocketConnect].dealCompleteMsgCallBack) {
        
        [JGJSocketRequest shareSocketConnect].dealCompleteMsgCallBack();
    }
}

+ (BOOL)filterNoMessageCountWithNormalMessage:(JGJChatMsgListModel *)normalMessage {
    
    if (normalMessage.chatListType) {
...
+ (void)dealOffLineWorkMsg:(NSArray *)workMsgs {
    ...
    for (int i = 0; i < workMsgs.count; i ++) {
        ...
        if (msgModel.chatListType ==  JGJChatListSyncNoticeTargetType || msgModel.chatListType ==  JGJChatListSyncProjectToYouType || msgModel.chatListType ==  JGJChatListJoinTeamType || msgModel.chatListType == JGJChatListCreateNewTeamType || msgModel.chatListType == JGJChatListSyncedSyncGroupToGroupType || msgModel.chatListType == JGJChatListAgreeSyncProjectToYouType || [msgModel.can_recive_client isEqualToString:@"manage"]) {// 排除脏数据
            ...
            if ([JGJSocketRequest shareSocketConnect].msg_idSet.count > 99) {
...
#pragma mark - 处理普通聊天的消息表
    ...
    if (msgModel.msg_total_type == JGJChatActivityMsgType) {// 活动
        ...
        groupModel.group_name = @"找活招工小助手";
        
    }else if (msgModel.msg_total_type == JGJChatNewFriendsMsgType) {// 招聘
...
+ (void)updateUnread_work_countWithGroupModel:(JGJChatGroupListModel *)groupModel chatListType:(JGJChatListType)type {
    ...
    switch (type) {
        ...
        case JGJChatListBillRecord://出勤公示
        {
            groupModel.unread_billRecord_count = @"1";
        }
            break;
...
+ (void)updateUnread_index_countWithGroupModelChatListType:(JGJChatListType)type indexModel:(JGJIndexDataModel *)indexModel{
    ...
    switch (type) {
        ...
        case JGJChatListApproveType:// 审批
        {
        ...
        case JGJChatListBillRecord://出勤公示
        {
            indexModel.unread_billRecord_count = @"1";
        }
            break;
        default:
            break;
...
+(NSString *)getUnreadMsgNum:(JGJChatGroupListModel *)groupModel msgModelMsg_type:(JGJChatMsgListModel *)msg_model{
    ...
    if ([self readGroupMsgWithMsgModel:msgModel]) {
        ...
        if ([msg_model.msg_sender isEqualToString:userId]) {// 发送者是自己 未读数不加1
            ...
            chat_unread_count = [NSString stringWithFormat:@"%ld",[originGroupModel.chat_unread_msg_count integerValue]];
...
- (DealCompleteMsgCallBack)dealCompleteMsgCallBack {
    
    return objc_getAssociatedObject(self, &dealCompleteMsg_callback_key);
}

- (void)setDealCompleteMsgCallBack:(DealCompleteMsgCallBack)dealCompleteMsgCallBack {
    
    objc_setAssociatedObject(self, &dealCompleteMsg_callback_key, dealCompleteMsgCallBack, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
