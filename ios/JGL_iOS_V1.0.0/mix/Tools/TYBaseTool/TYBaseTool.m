...
//
//        //没有注册就调到注册界面
//        if ((roleNum == 1 && !JLGMateIsInfoBool) || (roleNum == 2 && !JLGLeaderIsInfoBool)) {
...
//        }
        ...
        mateWorkitemsItem.accounts_type.code = [userInfo[@"accounts_code"] integerValue];
        mateWorkitemsItem.id = [userInfo[@"record_id"] integerValue];
        mateWorkitemsItem.role = roleNum;
        JGJRecordBillDetailViewController *workerGetBillVc = [[UIStoryboard storyboardWithName:@"JGJRecordBillDetailVC" bundle:nil] instantiateViewControllerWithIdentifier:@"JGJRecordBillDetailVC"];
        workerGetBillVc.sendMsgType = YES;
        workerGetBillVc.mateWorkitemsItems = mateWorkitemsItem;
        [navVc pushViewController:workerGetBillVc animated:YES];
    }
}
//跳转到记多天
...
//    NSString *year = [userInfo[@"year"] stringByAppendingString:@"-"];
//    NSString *dateString = [year stringByAppendingString:userInfo[@"month"]];
    
    //设置转换格式
    NSDate *date;
    if (userInfo[@"month"] && userInfo[@"year"]) {
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init] ;
    [formatter setDateFormat:@"yyyy-MM"];
    NSString *timeDate = [NSString stringWithFormat:@"%@-%@",userInfo[@"year"],userInfo[@"month"]];
    ...
    [navVc pushViewController:getIndexRecordVc animated:YES];

}
//设置全局的TextFiled光标
+ (void)setupGlobalTextField{
    [[UITextField appearance] setTintColor:AppFontd7252cColor];
}
