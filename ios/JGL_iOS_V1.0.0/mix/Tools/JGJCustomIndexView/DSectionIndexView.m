...
//        [_backgroundView.layer setLayerBorderWithColor:AppFont999999Color width:1 radius:8.0];
//        _backgroundView.backgroundColor = [UIColor whiteColor];
//        [self addSubview:self.backgroundView];
//        [self.backgroundView setHidden:YES];
        [self.layer setLayerBorderWithColor:AppFontdbdbdbColor width:1 radius:10.0];
...
- (void)layoutItemViews
{
    ...
    if (self.itemViewList.count) {
        itemViewHeight = (CGRectGetHeight(self.bounds) - offsetY * 2)/(CGFloat)(self.itemViewList.count);
    }
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    highlightedItemIndex = -1;
    if (self.touchCancelBlock) {
           self.touchCancelBlock(self, YES);
    }
}
