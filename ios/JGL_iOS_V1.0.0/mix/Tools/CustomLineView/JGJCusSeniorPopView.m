...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *onlineChatBtnHeight;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *messageLabelTopConstraints;
...
- (void)setIsMarkSingleBillComeIn:(BOOL)isMarkSingleBillComeIn {
    ...
    if (_isMarkSingleBillComeIn) {
        ...
        _messageLabelTopConstraints.constant = 0;
        _onlineChatBtnHeight.constant = 7;
        _onlineChatButton.hidden = YES;
        _messageLable.font = FONT(15);
        _messageLable.textColor = AppFont000000Color;
        _messageLable.textAlignment = NSTextAlignmentLeft;
        _seniorServiceButton.hidden = NO;
        [_seniorServiceButton setTitle:@"记账成功，数据永不丢失！" forState:(UIControlStateNormal)];
        [_seniorServiceButton setTitleColor:AppFonte15A153Color forState:(UIControlStateNormal)];
        _seniorServiceButton.titleLabel.font = FONT(17);
    }
}
