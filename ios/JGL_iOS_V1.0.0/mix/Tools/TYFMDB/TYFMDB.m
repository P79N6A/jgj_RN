...
+(NSArray *)getAllListCitys{
    ...
    NSString *searchSqlString = @"SELECT city_name,city_code,first_char,short_name,parent_id, city_url FROM jlg_city_data WHERE substr(city_code,5,2)=\'00\' AND substr(city_code,3,4)!=\'0000\'";
    FMResultSet* resultSet = [database executeQuery:searchSqlString];
    NSMutableArray *dataArray = [NSMutableArray array];
    // 逐行读取数据
    while ([resultSet next])
...
+ (NSArray *)searchCalendarTable:(NSString *)tableName sqlite:(NSString *)sqlite {
    ...
    FMDatabase *database = [TYFMDB getCalendarDataBase];
    FMResultSet *resultSet = [self getResultSetByDataBase:database TableName:tableName sqlite:sqlite];
    NSMutableArray *dataArray = [NSMutableArray array];
    // 逐行读取数据
    while ([resultSet next])
    {
...
//获取database
+ (FMDatabase *)getCalendarDataBase{
    FMDatabase* database;
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"huangli" ofType:@".db"];
...
//获取resultSet
+ (FMResultSet *)getResultSetByDataBase:(FMDatabase *)database TableName:(NSString *)tableName sqlite:(NSString *)sqlite {
    
    if (![self dataBase:database isExitsTable:tableName]) {
        return nil;
    }
    NSString *searchSqlString = [NSString stringWithFormat:@"%@",sqlite];
    FMResultSet* resultSet = [database executeQuery:searchSqlString];
    return resultSet;
}
@end
