...
#define ForegroundStarImage @"sel_star_icon"
#define BackgroundStarImage @"gray_star_icon"
...
- (void)userTapRateView:(UITapGestureRecognizer *)gesture {
    ...
    [self setStartScoreWithPoint:tapPoint.x];
    
    TYLog(@"tapPoint----------%lf", tapPoint.x);
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (offset < (self.bounds.size.width / self.numberOfStars)) {
        
        offset = (self.bounds.size.width / self.numberOfStars);
    }
    
    [self setStartScoreWithPoint:offset];
    
}

- (void)setStartScoreWithPoint:(CGFloat)offset {
    
    if (self.isForbidTouch) {
    ...
    switch (_rateStyle) {
        case WholeStar:
        {
            ...
            //                self.currentScore = 0;
            //
            //            }else if (self.currentScore == 0 && ceilf(realStarScore) == 1) {
            //
            //                self.currentScore = 1;
            //
            //            }else {
