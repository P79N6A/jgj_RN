...
- (void)setProListModel:(JGJMyWorkCircleProListModel *)proListModel {
    ...
    if ([proListModel.class_type isEqualToString:@"team"]) { //项目组
        [self.avatarGroupImageView getRectImgView:proListModel.members_head_pic];
//        JGJHeadView *head = [[JGJHeadView alloc]initWithFrame:CGRectMake(0, 0, 51, 51) withframe:proListModel.members_head_pic];
//        [self.avatarGroupImageView addSubview:head];
        self.groupFlagImageView.hidden = YES;
    } else {
       // [self.avatarGroupImageView getRectImgView:proListModel.members_head_pic];
//        JGJHeadView *head = [[JGJHeadView alloc]initWithFrame:CGRectMake(0, 0, 51, 51) withframe:proListModel.members_head_pic];
//        [self.avatarGroupImageView addSubview:head];
    ...
    self.groupName.text = proListModel.pro_name;
    ...
    } else if (groupNameWidth > MaxGroupNameWidth && self.groupName.text.length >11) {
        
        self.groupName.text = [self.groupName.text substringToIndex:11];
        self.groupName.text = [self.groupName.text stringByAppendingFormat:@"%@(%@)", @"...", proListModel.members_num];
