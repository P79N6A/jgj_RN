...
-(void)didMoreselectweaterevent:(NSIndexPath *)indexpath andArr:(NSMutableArray *)selectArr andDelete:(BOOL)del
{
    if (tag == 1) {
        ...
        if (selectArr.count==1) {
            ...
            if ([selectArr.lastObject isEqualToString:selectArr.firstObject]) {
                
                weatherStr = [NSString stringWithFormat:@"%@",selectArr.firstObject];
    ...
    if ([buttonTitle isEqualToString:@"确定"]) {
        if (tag == 1) {
            if (_morningArr.count >= 2) {
                ...
                if ([_morningArr[0] isEqualToString:_morningArr[1]]) {
                    
                    _sunylablem.text = [NSString stringWithFormat:@"%@",_morningArr.firstObject];
                }else {
