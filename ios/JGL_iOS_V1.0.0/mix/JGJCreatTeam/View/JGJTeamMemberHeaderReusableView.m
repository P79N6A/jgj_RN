...
- (void)setCommonModel:(JGJTeamMemberCommonModel *)commonModel {
    ...
    self.teamMemberNumberLable.text = [NSString stringWithFormat:@"(%@)", @(commonModel.count)];
    self.teamMemberTypeLable.text = commonModel.headerTitle;
    self.lineView.hidden = _commonModel.teamControllerType == JGJCreatTeamControllerType;
    BOOL isHidden = _commonModel.teamControllerType == JGJCreatTeamControllerType || _commonModel.teamControllerType == JGJAddProSourceMemberControllerType;
    
    self.teamMemberNumberLable.hidden = isHidden;
    
    self.sourceMemberDesButton.hidden = isHidden;
    
    self.desLineView.hidden = isHidden;
    
    if (_commonModel.teamControllerType == JGJCreatTeamControllerType) {
        
        NSString *tips = @"(点击头像可删除)";
        
        self.teamMemberTypeLable.text = [NSString stringWithFormat:@"%@%@", @"班组成员", tips];
        
        [self.teamMemberTypeLable markText:tips withColor:AppFont999999Color];
        
    }else {
        
        self.teamMemberNumberLable.hidden = YES;
        ...
        if (commonModel.memberType == JGJProSourceMemberType && commonModel.teamControllerType != JGJAddProSourceMemberControllerType) { //数据来源人的数量显示在左边
            ...
            self.teamMemberNumberLable.hidden = NO;
            
            self.teamMemberTypeLable.hidden = NO;
