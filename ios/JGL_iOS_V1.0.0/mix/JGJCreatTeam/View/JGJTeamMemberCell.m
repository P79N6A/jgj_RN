...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    teamMemberModel.classType = self.commonModel.classType;
    cell.teamMemberModel = teamMemberModel;
    cell.delegate = self;
    return cell;
}
...
//    点击判断是不是我们平台成员弹框
    ...
    JGJSynBillingModel *teamMemberModel = nil;
    
    if (self.teamMemberModels.count > 0) {
        
        teamMemberModel = self.teamMemberModels[indexPath.row];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isCheckPlanHeader && kind == UICollectionElementKindSectionHeader) {
    ...
    }else if (kind == UICollectionElementKindSectionHeader && self.commonModel.memberType == JGJProMemberType && ![self.teamInfo.group_info.class_type isEqualToString:@"group"]) {
        ...
        CGFloat height = self.commonModel.memberType == JGJReportMemberType ? HeaderHegiht : HeaderHegiht / 2.0;
        
        if (self.commonModel.teamControllerType == JGJGroupMangerControllerType) {
            
            //3.3.4隐藏
            headerView.hidden = self.commonModel.memberType != JGJReportMemberType;
            
             self.collectionViewLayout.headerReferenceSize = CGSizeMake(TYGetUIScreenWidth, height);
        }
