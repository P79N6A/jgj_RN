...
//    [UIView maskLayerTarget:self.headButton roundCorners:UIRectCornerAllCorners cornerRad:CGSizeMake(5, 5)];
    ...
    self.headButton.backgroundColor = AppFontf1f1f1Color;
...
- (void)setTeamMemberModel:(JGJSynBillingModel *)teamMemberModel {
    ...
    self.createrFlagImageView.hidden = self.commonModel.teamControllerType == JGJCreatTeamControllerType;
    
    if ([teamMemberModel.is_creater isEqualToString:@"1"]) {
        
        creatImageStr = @"icon_creator";
        
    }else if (teamMemberModel.is_agency && !teamMemberModel.isMangerModel) {
        
        creatImageStr = @"agency_member_icon";
...
- (void)handleTeamMemberHeader:(JGJSynBillingModel *)teamMemberModel {
    ...
    [self.headButton setMemberPicButtonWithHeadPicStr:teamMemberModel.head_pic memberName:teamMemberModel.real_name memberPicBackColor:teamMemberModel.modelBackGroundColor membertelephone:teamMemberModel.telephone];
}
