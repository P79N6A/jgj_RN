...
//    if ([self.detailTF.text containsString:@"."]) {
//    self.detailTF.maxLength = 9;
//    }else{
    self.detailTF.maxLength = 6;
//    }
    
//     self.detailTF.maxLength = 6;//2.1.0 - yj修改设置工资标准每日标准工资输入8位99999999显示错误
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{  //string就是此时输入的那个字符textField就是此时正在输入的那个输入框返回YES就是可以改变输入框的值NO相反
    ...
    if (![NSString isEmpty:string]) {
        ...
        if ([string isEqualToString:@"."] && range.location < self.detailTF.text.length ) {
            
            return NO;
        }
        
        if ([self.detailTF.text containsString:@"."] && self.detailTF.text.length == range.location) {
