...
WCDB_SYNTHESIZE(JGJChatGroupListModel, agency_group_user)
...
WCDB_SYNTHESIZE(JGJChatGroupListModel, unread_bill_count)
WCDB_SYNTHESIZE(JGJChatGroupListModel, work_message_num)
...
- (CGFloat)checkProCellHeight {
    ...
    BOOL isMySelfGroup = [self.creater_uid isEqualToString:user_id] || [self.agency_group_userInfo.uid isEqualToString:user_id];
...
//    NSString *proName = [NSString stringWithFormat:@"%@ %@", self.group_name, myGroupFlagStr];
    ...
    if (!self.isCheckClosedPro) {
        ...
        _checkProCellHeight = [NSString stringWithContentWidth:maxW content:proName font:AppFont30Size lineSpace:1] + 42;
...
- (JGJSynBillingModel *)agency_group_userInfo {
    
    NSDictionary *dic = [self.agency_group_user mj_JSONObject];
    
    return [JGJSynBillingModel mj_objectWithKeyValues:dic];
}
