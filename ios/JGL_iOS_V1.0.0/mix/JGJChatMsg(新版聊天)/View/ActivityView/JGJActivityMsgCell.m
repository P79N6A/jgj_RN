...
- (void)awakeFromNib {
    ...
    self.act_des.preferredMaxLayoutWidth = TYGetUIScreenWidth - 20;
    self.act_title.preferredMaxLayoutWidth = TYGetUIScreenWidth - 20;
    
    self.act_title.textContainerInset = UIEdgeInsetsMake(0, 15, -5, 15);
...
- (void)subClassSetWithModel:(JGJChatMsgListModel *)jgjChatListModel{
    ...
    if (![NSString isEmpty:self.act_title.text]) {
        
        NSMutableAttributedString *attributeText = [[NSMutableAttributedString alloc] initWithString:self.act_title.text attributes:@{NSShadowAttributeName:shadow}];
        self.act_title.attributedText = attributeText;
    }
    
    
    self.activity_time.text = [NSDate chatShowActivityDateWithTimeStamp:jgjChatListModel.send_time];
