...
+ (BOOL)isExistMsgLocalIdModelWithMsgModel:(JGJChatMsgListModel *)msgModel {
    ...
    if (![NSString isEmpty:msgModel.local_id] && ![msgModel.local_id isEqualToString:@"0"]) {
        ...
        msgModel = [msgDBManger.table getOneObjectWhere:JGJChatMsgListModel.local_id == msgModel.local_id && [self comConWithChatMsgModel:msgModel]];
        
        is_exist = ![NSString isEmpty:msgModel.local_id];
...
+(BOOL)isExistMsgIdModel:(JGJChatMsgListModel *)msgModel {
    ...
    NSString *user_id = [TYUserDefaults objectForKey:JLGUserUid];
    JGJChatMsgListModel *existModel = [msgDBManger.table getOneObjectWhere:JGJChatMsgListModel.msg_id == msgModel.msg_id and JGJChatMsgListModel.user_unique == user_id];
...
+(BOOL)isExistMsgBill_IdModel:(JGJChatMsgListModel *)msgModel {
    ...
    NSString *user_id = [TYUserDefaults objectForKey:JLGUserUid];
    JGJChatMsgListModel *existModel = [msgDBManger.table getOneObjectWhere:JGJChatMsgListModel.bill_id == msgModel.bill_id and JGJChatMsgListModel.user_unique == user_id];
    
    BOOL is_exist = ![NSString isEmpty:existModel.bill_id];
...
+ (NSArray *)getWorkMsgModelsWithChatMsgListModel:(JGJChatMsgListModel *)msgListModel {
    ...
    WCTCondition condition = JGJChatMsgListModel.msg_total_type == msgListModel.msg_total_type && JGJChatMsgListModel.user_unique == user_id && JGJChatMsgListModel.msg_type != @"syncProjectToYou" && JGJChatMsgListModel.msg_type != @"joinTeam" && JGJChatMsgListModel.msg_type != @"createNewTeam" && JGJChatMsgListModel.can_recive_client != @"manage";
    if(msg_id == 0) {
        
        msgModels = [msgDBManger.table getObjectsWhere:condition orderBy:JGJChatMsgListModel.send_time.order(WCTOrderedDescending) limit:JGJChatPageSize];
        
    }else {
        
        msgModels = [msgDBManger.table getObjectsWhere:condition &&  JGJChatMsgListModel.wcdb_msg_id < msg_id orderBy:JGJChatMsgListModel.send_time.order(WCTOrderedDescending) limit:JGJChatPageSize];
    }
...
+ (JGJChatMsgListModel *)containSpecialWordsWithMsgModel:(JGJChatMsgListModel *)msgModel {
    ...
    for (NSString *word in coverwordArr) {
        ...
        if ([msgModel.msg_text containsString:word]) {
            if (![msgModel.msg_type isEqualToString:@"signIn"]) {
                is_contain = YES;
                break;
            }
...
+ (BOOL)deleteAllChatMsgListData {
    ...
    return [msgDBManger.table deleteAllObjects];
}
