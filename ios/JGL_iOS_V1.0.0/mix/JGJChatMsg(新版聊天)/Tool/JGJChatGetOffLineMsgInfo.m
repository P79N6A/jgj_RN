...
+ (void)http_getChatIndexList{
    ...
    [JLGHttpRequest_AFN PostWithNapi:@"chat/get-index-list" parameters:nil success:^(id responseObject) {
        ...
        TYLog(@"首页消息 responseObject = %@",responseObject);
        ...
        indexModel.agency_group_userInfo = [responseObject[@"group_info"][@"agency_group_user"] mj_JSONString];
        ...
        if (maneger.getIndexListFailed) {
            
            maneger.getIndexListFailed();
        }
    }];
}
...
+ (void)http_getChatGroupListSuccess:(void (^)(BOOL responseObject))success {
    ...
    [JLGHttpRequest_AFN PostWithNapi:@"chat/get-chat-list" parameters:nil success:^(id responseObject) {
        ...
        for (int i = 0; i < list.count; i++) {
            ...
            [agencyDic setValue:listModel.agency_group_user.uid forKey:@"uid"];
            [agencyDic setValue:listModel.agency_group_user.end_time forKey:@"end_time"];
            [agencyDic setValue:@(listModel.agency_group_user.is_expire) forKey:@"is_expire"];
            [agencyDic setValue:@(listModel.agency_group_user.is_start) forKey:@"is_start"];
            [agencyDic setValue:listModel.agency_group_user.start_time forKey:@"start_time"];
            
            groupModel.agency_group_user = [agencyDic mj_JSONString];
...
//        //校准未读数每次启动只传一次暂时不用了，存本地根据推送加1
//
//        [self calibrationGroupMsgUnreadNum:list];
...
+ (void)calibrationGroupMsgUnreadNum:(NSArray *)groups {
    
    //保证每次启动调用一次
    
    if (JGJAppIsDidCallBackGroupsBool) {
        
        return;
    }
    
    NSMutableArray *groupMutabs = [NSMutableArray array];
    
    for (JGJHttpGetChatListModel *listModel in groups) {
        
        if ([listModel isMemberOfClass:[JGJHttpGetChatListModel class]]) {
            
            NSString *class_type = listModel.class_type;
            
            NSString *group_id = listModel.group_id;
            
            if (![NSString isEmpty:class_type] && ![NSString isEmpty:group_id]) {
                
                NSDictionary *groupDic = @{@"group_id" : group_id,
                                           @"class_type" : class_type
                                           };
                
                [groupMutabs addObject:groupDic];
    ...
    if (groupMutabs.count > 0) {
     
        NSString *groupsStr = [groupMutabs mj_JSONString];
        
        if ([NSString isEmpty:groupsStr]) {
        ...
        [JLGHttpRequest_AFN PostWithNapi:@"chat/calibration-msg-unread-num" parameters:parameters success:^(id responseObject) {
            
            TYLog(@"校准未读数 -------%@ \n groupsCount----%@", parameters, @(groups.count));
            
            [TYUserDefaults setBool:YES forKey:JGJAppIsDidCallBackGroups];
...
#pragma mark - 切换项目
+ (void)http_gotoTheGroupHomeVCWithGroup_id:(NSString *)group_id class_type:(NSString *)class_type isNeedChangToHomeVC:(BOOL)needChange isNeedHttpRequest:(BOOL)needRequest success:(void (^)(BOOL))success{
...
+ (JGJIndexDataModel *)getTheIndexDataModelWithGroup_id:(NSString *)group_id classType:(NSString *)class_type {
    ...
    indexModel.unread_bill_count = groupModel.unread_bill_count;
    ...
    indexModel.agency_group_userInfo = groupModel.agency_group_user;
...
+ (void)refreshIndexTbToHomeVC {
    ...
    proListModel.unread_bill_count = indexModel.unread_bill_count;
    ...
    // 更新聊聊表对应的班组数据
    groupModel.unread_task_count = indexModel.unread_task_count;
    groupModel.unread_inspect_count = indexModel.unread_inspect_count;
    groupModel.unread_notice_count = indexModel.unread_notice_count;
    groupModel.unread_safe_count = indexModel.unread_safe_count;
    groupModel.unread_sign_count = indexModel.unread_sign_count;
    groupModel.unread_log_count = indexModel.unread_log_count;
    groupModel.unread_quality_count = indexModel.unread_quality_count;
    groupModel.unread_billRecord_count = indexModel.unread_billRecord_count;
    groupModel.unread_bill_count = indexModel.unread_bill_count;
    groupModel.unread_approval_count = indexModel.unread_approval_count;
    groupModel.unread_weath_count = indexModel.unread_weath_count;
    groupModel.unread_meeting_count = indexModel.unread_meeting_count;
    groupModel.work_message_num = indexModel.work_message_num;
    groupModel.chat_unread_msg_count = groupModel.chat_unread_msg_count;
    
    [JGJChatMsgDBManger updateChatGroupListTableTheUnread_work_countWithGroupListModel:groupModel group_id:groupModel.group_id class_type:groupModel.class_type];
    
    
    NSDictionary *dic = [indexModel.agency_group_userInfo mj_JSONObject];
    proListModel.group_info.agency_group_user = [JGJSynBillingModel mj_objectWithKeyValues:dic];
...
+ (void)http_getHomeCalendarData {
    ...
    NSString *date = [NSDate stringFromDate:[NSDate date] format:@"yyyyMM"];
    [JLGHttpRequest_AFN PostWithNApiReturnTask:@"workday/worker-month-total" parameters:@{@"date":date} success:^(id responseObject) {
        ...
        if (maneger.getHomeCalendarDataSuccess) {
            
            maneger.getHomeCalendarDataSuccess();
        }
    ...
    // 获取到当前时间的前一个月
    NSString *previousMonth = [NSDate stringFromDate:[NSDate getPriousorLaterDateFromDate:[NSDate date] withMonth:-1] format:@"yyyyMM"];
