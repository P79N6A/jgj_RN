...
- (void)awakeFromNib {
    ...
    self.titleLable.preferredMaxLayoutWidth = TYGetUIScreenWidth - 183;
}
...
- (void)setNotifyModel:(JGJNewNotifyModel *)notifyModel {
    ...
    //优质班组长和付款成功隐藏按钮
    self.orderButton.hidden = notifyModel.isSuccessSyn || notifyModel.notifyType == superior_work_leader;
    ...
    if (notifyModel.isSuccessSyn || notifyModel.notifyType == superior_work_leader) {
    ...
    if (notifyModel.notifyType == work_leader_certify || notifyModel.notifyType == superior_work_leader) {
        
        [self setWorkLeaderCertifyWithNotifyModel:notifyModel];
    }
    
}

#pragma mark - 班组长认证 和 优质班组长
- (void)setWorkLeaderCertifyWithNotifyModel:(JGJNewNotifyModel *)notifyModel {

    NSString *imageStr = @"work_leader_certify_icon";
    
    if (notifyModel.notifyType == superior_work_leader) {
        
        imageStr = @"superior_work_leader_icon";
        
    }
    
    self.flagImageView.image = [UIImage imageNamed:imageStr];
    
    [self.orderButton setTitle:@"查看详情" forState:UIControlStateNormal];
...
- (IBAction)orderButtonPressed:(UIButton *)sender {
    ...
    ProiClodInsuffRemindCellButtonType buttonType = ProiClodInsuffRemindCellOrderButtonType;
    
    if (_notifyModel.notifyType == superior_work_leader || _notifyModel.notifyType == work_leader_certify) {
        
        buttonType = CheckDetailButtonType;
    }
    
    if ([self.delegate respondsToSelector:@selector(handleJGJProiClodInsuffRemindCellWithNotifyModel:buttonType:)]) {
        
        [self.delegate handleJGJProiClodInsuffRemindCellWithNotifyModel:self.notifyModel buttonType:buttonType];
    }
