...
//            [self.headImageView getCircleImgView:notifyModel.members_head_pic];
//            JGJHeadView *head = [[JGJHeadView alloc]initWithFrame:CGRectMake(0, 0, 51, 51) withframe:notifyModel.members_head_pic];
//            [self.headImageView addSubview:head];
        } else {
            [self.headImageView getRectImgView:notifyModel.members_head_pic];
//            JGJHeadView *head = [[JGJHeadView alloc]initWithFrame:CGRectMake(0, 0, 51, 51) withframe:notifyModel.members_head_pic];
//            [self.headImageView addSubview:head];
...
//    notifyModel.group_name = @"测试班组名字";
//    notifyModel.user_name = @"测试姓名";
...
//       其余类型待特殊处理
        ...
        case NewBillingType:{
            synButtonTitle = @"加入班组";
            if (notifyModel.isJoinTeam) {
                self.notifyTypeFlagImageView.hidden = NO;
                self.synBillingButton.hidden = YES;
                self.synButtonWidth.constant = 0;
                self.refuseButtonWidth.constant = 10;
                self.refuseButton.hidden = YES;
                self.notifyTypeFlagImageView.image = [UIImage imageNamed:@"joinTeam"];
            } else {
                self.synBillingButton.hidden = NO; //加入班组按钮 和同步按钮复用
                self.synButtonWidth.constant = SynButtonW;
                self.accounterFlag.hidden = NO;
                self.refuseButtonWidth.constant = RefuseButtonW;
                self.refuseButton.hidden = YES;
            }
            mergeStr = [NSString stringWithFormat:@"有新工友 %@ 对你记了一笔帐", notifyModel.user_name];
            textArray = @[notifyModel.user_name];
        }
            break;
    ...
    [self.detailLable markattributedTextArray:textArray color:AppFont333333Color];
}
