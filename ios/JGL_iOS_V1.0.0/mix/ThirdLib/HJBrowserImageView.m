...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
      ...
      /*
        UILongPressGestureRecognizer *saveLongGuessTure = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(savePicImage)];
    
        [self addGestureRecognizer:saveLongGuessTure];
       */
...
{
    if (!actinSheet) {
    actinSheet = [[UIActionSheet alloc]initWithTitle:@"确认要保存图片到相册中?" delegate:self cancelButtonTitle:nil destructiveButtonTitle:@"确定" otherButtonTitles:@"取消", nil];
...
{
    if (buttonIndex == 0) {
        UIImageWriteToSavedPhotosAlbum(_scrollImageView.image, nil,nil, NULL);
