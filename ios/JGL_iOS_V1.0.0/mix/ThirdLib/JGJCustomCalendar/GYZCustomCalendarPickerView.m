...
- (instancetype)initWithGregorianCalendarTitle:(NSString *)calendarTitle IDJCalendar:(IDJCalendar *)calendar {
...
//        }
    ...
    if (calendarType == GregorianCalendar) {
        ...
        _selectRow = [years indexOfObject:cal.year];
        _selectComponent = 0;
        UILabel *yearLable = (UILabel *)[_pickerView viewForRow:[years indexOfObject:cal.year] forComponent:0];
        yearLable.textColor = AppFontd7252cColor;
    } else if (calendarType == ChineseCalendar) {
...
//        [_pickerView selectRow:[years indexOfObject:[NSString stringWithFormat:@"%@-%@-%@", cal.era, ((IDJChineseCalendar *)cal).jiazi, cal.year]] inComponent:0 animated:YES];
    ...
    _selectRow = [months indexOfObject:cal.month];
    _selectComponent = 1;

    [_pickerView selectRow:[days indexOfObject:cal.day] inComponent:2 animated:YES];
    _selectRow = [days indexOfObject:cal.day];
    _selectComponent = 2;
    
    UILabel *monthLable = (UILabel *)[_pickerView viewForRow:[months indexOfObject:cal.month] forComponent:1];
    monthLable.textColor = AppFontd7252cColor;
    
    UILabel *dayLable = (UILabel *)[_pickerView viewForRow:[days indexOfObject:cal.day] forComponent:2];
    dayLable.textColor = AppFontd7252cColor;
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    mycom1.textColor = AppFont999999Color;
    switch (component) {
        case 0:{
        ...
        case 2:{
            if (self.calendarType == GregorianCalendar) {
               ...
               int chineseWeekday = [IDJCalendarUtil weekDayWithChineseYear:[cal.year intValue] month:cal.month day:[[days objectAtIndex:row]intValue]];
                ...
                UILabel *selectedLable = (UILabel *)[_pickerView viewForRow:row forComponent:component];
...
//    if (row == _selectRow || component == _selectComponent) {
//        UILabel *selectedLable = (UILabel *)[_pickerView viewForRow:_selectRow forComponent:_selectComponent];
//        selectedLable.textColor = AppFontd7252cColor;
//    }
...
- (void)changeDays{
    ...
    NSUInteger cellRow=[days indexOfObject:cal.day];
    //假如用户上次选择的是1月31日，当月份变为2月的时候，第三列的滚轮不可能再选中31日，我们设置默认的值为第一个。
    if (cellRow==NSNotFound) {
        cellRow=0;
        cal.day=[days objectAtIndex:0];
    }
    _selectRow = cellRow;
    _selectComponent = 2;
    [_pickerView selectRow:cellRow inComponent:2 animated:YES];
}
...
- (void)selectCalendarClick:(UIButton *)sender {
    if (IsMinYear) {
        [TYShowMessage showError:@"当前时间为最低年限"];
        return;
    }
    if (IsMaxYear) {
        [TYShowMessage showError:@"当前时间为最高年限"];
        return;
    }
