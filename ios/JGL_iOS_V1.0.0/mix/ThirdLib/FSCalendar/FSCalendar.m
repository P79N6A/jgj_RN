...
//2.1.0添加修改
//#define KJGJFSCalendar_2_1_0 YES  _header.selectWeekDate = _selectedDates.firstObject;

static const NSInteger FSCalendarExtraDay = 0;//需要额外加的天数
static const NSInteger CollectionViewRowNum = 6;//一共多少行
static const CGFloat ScopeWeekHeight = 0;//如果是周的时候的高度
...
@property (nonatomic, assign) BOOL isHaveShowHoldView;
...
#ifdef KJGJFSCalendar_2_1_0

    [collectionView registerClass:[JGJFSCalendarCell class] forCellWithReuseIdentifier:@"JGJFSCalendarCell"];
#else
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        // Tony修改过的地方，修改月历的头部显示
        /*
        CGFloat headerHeight = _scope == FSCalendarScopeMonth?self.preferedHeaderHeight:ScopeWeekHeight;//此处修改为取消日历的顶部
//        CGFloat headerHeight = self.preferedHeaderHeight;
*/
        CGFloat headerHeight;
        if (_showHeader) {
        
            headerHeight = self.preferedHeaderHeight-5;
        }else{
       
            headerHeight = _scope == FSCalendarScopeMonth?self.preferedHeaderHeight:ScopeWeekHeight;
            
#pragma mark -修改记工首页的适配u
            if (_mainVC) {

                headerHeight = headerHeight - 10;
            }
            
            if (_headerNeedOffset) {
        ...
        _header.scrollDirection = UICollectionViewScrollDirectionVertical;
        ...
        [_weekdays enumerateObjectsUsingBlock:^(UILabel *weekdayLabel, NSUInteger index, BOOL *stop) {
            ...
            weekdayLabel.textColor = (index == 0)||(index == _weekdays.count - 1)?JGJMainColor:_appearance.weekdayTextColor;
        }];
        
        if (_scope == FSCalendarScopeWeek || _notShowWeakDayLabel) {
            
            [self.lineLable removeFromSuperview];
            
        }else {
            
            self.lineLable.frame = CGRectMake(0, _header.fs_height + self.preferedWeekdayHeight, _header.fs_width, 0.5);
            self.lineLable.backgroundColor = [AppFontdbdbdbColor colorWithAlphaComponent:0.5];
            [_contentView addSubview:self.lineLable];
...
#pragma mark - 添加分割线
        ...
        if (_scope != FSCalendarScopeWeek) {
            if (headerPlaceView) {
                [headerPlaceView removeFromSuperview];
            }
            headerPlaceView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(_deliver.frame ) - 1, TYGetUIScreenWidth, .5)];
            headerPlaceView.backgroundColor = AppFontdbdbdbColor;
            
            [_deliver addSubview:headerPlaceView];
        ...
        if (!self.floatingMode) {
            ...
            // Tony修改过的地方
                    CGFloat contentHeight = rowHeight*CollectionViewRowNum + padding*2;
...
- (void)setNotShowWeakDayLabel:(BOOL)notShowWeakDayLabel {
    
    _notShowWeakDayLabel = notShowWeakDayLabel;
    if (_notShowWeakDayLabel) {
...
{
    //Tony修改过的地方
    CGFloat headerHeight = _scope == FSCalendarScopeMonth?self.preferedHeaderHeight:ScopeWeekHeight;
    CGFloat weekdayHeight = self.preferedWeekdayHeight;
    CGFloat rowHeight = self.preferedRowHeight;
    ...
    if (!self.floatingMode) {
        switch (scope) {
            case FSCalendarScopeMonth: {
                CGFloat height = weekdayHeight + headerHeight + CollectionViewRowNum*rowHeight + paddings;
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView
{
    ...
    switch (_scope) {
        case FSCalendarScopeMonth:
            return [self monthsFromDate:[self beginingOfMonthOfDate:_minimumDate] toDate:_maximumDate] + 1;
        case FSCalendarScopeWeek:{
            NSInteger section = [self weeksFromDate:[self beginingOfWeekOfDate:_minimumDate] toDate:_maximumDate] + 1;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    if (self.collectionViewLayout.transition == FSCalendarTransitionWeekToMonth && self.collectionViewLayout.state == FSCalendarTransitionStateInProgress) {
        return CollectionViewRowNum*7 + FSCalendarExtraDay;
    }
    if (!self.floatingMode) {
        switch (_scope) {
            case FSCalendarScopeMonth: {
        ...
        NSInteger numberOfPlaceholdersForPrev = (currentWeekday-_firstWeekday + FSCalendarExtraDay) % 7;
        NSInteger numberOfRows = (numberOfPlaceholdersForPrev+numberOfDaysInMonth)/7 + ((numberOfPlaceholdersForPrev+numberOfDaysInMonth)%7!=7);
        return numberOfRows * 7;
    }
    return 7;
}

- (void)setCc_selectedDate:(NSDate *)cc_selectedDate {
    
    _cc_selectedDate = cc_selectedDate;
...
{
#ifdef KJGJFSCalendar_2_1_0
    JGJFSCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"JGJFSCalendarCell" forIndexPath:indexPath];
#else
    ...
    [self cleanCellSubViewandCell:cell];

#endif
    return cell;
}
-(void)cleanCellSubViewandCell:(FSCalendarCell *)cell
{
    for (UIView *obj in cell.subviews) {
        if (obj.tag == 120||obj.tag == 10||obj.tag == 1122) {
    ...
    if (_scope == FSCalendarScopeWeek) {
        ...
        for (FSCalendarCell *cells in _collectionView.visibleCells) {
            if (!cells.dateIsSelected) {
                cells.contentView.backgroundColor = [UIColor whiteColor];
                cells.backgroundLayer.strokeColor = [UIColor whiteColor].CGColor;
            }
            
            for (UIView *view in cells.subviews) {
...
{
#ifdef KJGJFSCalendar_2_1_0
    JGJFSCalendarCell *cell = (JGJFSCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
#else
    FSCalendarCell *cell = (FSCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
    
    if (self.moreDayCalendar) {
    ...
    if (_selectShow && !_rainCalendar) {
        
        cell.placeView.layer.borderWidth = 0;
        cell.showHoldView = YES;
    ...
    for (FSCalendarCell *cells in  _collectionView.visibleCells) {
        ...
        cells.recordedImageView.hidden = YES;
        cells.leftRecordedImageView.hidden = YES;
    }
    ...
    if (_rainCalendar) {
        ...
        for (FSCalendarCell *cells in  _collectionView.visibleCells) {
            ...
            if (_rainCalendar &&!cells.upleftimageView.image) {
            
                cells.contentView.backgroundColor = [UIColor whiteColor];
           
            }
        
            cells.departLable.hidden = NO;
    ...
    if (self.moreDayCalendar) {
        ...
        selectedModel.date = cell.date;
        selectedModel.type = self.contractorType;
        [self.contractorTypeSelectedArray addObject:selectedModel];
    }
    ...
    self.header.selectWeekDate = selectedDate;
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
...
#endif
    if (_rainCalendar) {
        for (UITableViewCell *cells in collectionView.visibleCells) {
        ...
        if (_scope == FSCalendarScopeWeek) {
            for (UITableViewCell *cells in collectionView.visibleCells) {
                ...
                for (UIView *view in cells.subviews) {
                    if ( view.tag == 20||view.tag == 120||view.tag == 10) {
    ...
    if ([self isDateSelected:targetDate]) {
        // 这个if几乎不会调用到  && [self.selectedDates containsObject:targetDate] 后期添加这一段  有异常
        if (self.allowsMultipleSelection && [self.selectedDates containsObject:targetDate]) {
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.moreDayCalendar) {
        ...
        for (int i = 0; i < self.contractorTypeSelectedArray.count; i ++) {
            
            JGJMoreDaySelectedModel *selectedModel = self.contractorTypeSelectedArray[i];
            if ([[selectedModel.date timestamp] isEqualToString:[cell.date timestamp]]) {
                
                [self.contractorTypeSelectedArray removeObject:selectedModel];
                break;
            }
        }
    }
    ...
    if (self.selectShow && !_rainCalendar) {
        
        cell.showHoldView = NO;
    }
    
//    [self jugdeIsHaveShowHoldView];
    [self.collectionView reloadData];

#pragma mark -此处为取消选中效果
...
- (void)jugdeIsHaveShowHoldView {
    
    // 在一天记多人里面
    // 1.第一次循环 找到显示的cell中是否还有被显示的holdview -> [self startanimationwithcell:cell] 在这个方法里面生成的;
    _isHaveShowHoldView = NO;
    for (FSCalendarCell *cell in _collectionView.visibleCells) {

        if (cell.showHoldView) {

            _isHaveShowHoldView = YES;
    ...
    // 2.第二次循环 要是没有显示的有holdView 则把有cell.subtitle对应的cell.recordedImageView显示出来
    if (!_isHaveShowHoldView) {

        for (FSCalendarCell *cell in  _collectionView.visibleCells) {

            if (self.selectShow && !_rainCalendar) {
                if (![NSString isEmpty:cell.subtitle]) {

                    cell.placeView.layer.masksToBounds = YES;
                    cell.placeView.layer.borderWidth = 1;
                    cell.placeView.layer.borderColor = AppFontF9A00FColor.CGColor;

                }else{

                    cell.placeView.layer.masksToBounds = YES;
                    cell.placeView.layer.borderWidth = 1;
                    cell.placeView.layer.borderColor = [UIColor clearColor].CGColor;
                }
            }
        }
        [self.collectionView reloadData];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(calendarCurrentPageWillChange:)]) {
        [_delegate calendarCurrentPageWillChange:self];
    }
}
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    NSDate *currentPageDate = self.currentPage;
    ...
    if (shouldTriggerPageChange) {
        ...
        self.isAddDays = [NSDate getDaysFrom:currentPageDate withToDate:_currentPage] < 0?-1:1;
        [self currentPageDidChange];
        [self didChangeValueForKey:@"currentPage"];
    }
    
    
#pragma mark - lyq添加 用于清除状态
    if (self.delegate &&[self.delegate respondsToSelector:@selector(calendarCurrentScopeWillChange:)]) {
        [self.delegate calendarCurrentScopeWillChange:self];
    }
...
- (void)disChangeMonthDyDate:(NSDate *)date{
    ...
    NSDate *currentPageDate = self.currentPage;
    //修改选中的时间
    NSInteger dateMonths = [NSDate getMonthsFrom:currentPageDate withToDate:date];

    CGFloat currentOffset = 0, contentSize = 0;
    switch (_collectionViewLayout.scrollDirection) {
        case UICollectionViewScrollDirectionHorizontal: {
            currentOffset = self.collectionView.contentOffset.x;
            contentSize = self.collectionView.fs_width;
            break;
        }
        case UICollectionViewScrollDirectionVertical: {
            currentOffset = self.collectionView.contentOffset.y;
            contentSize = self.collectionView.fs_height;
            break;
        }
    }
    ...
    NSDate *minimumPage = [self beginingOfMonthOfDate:_minimumDate];
    _currentPage = [self dateByAddingMonths:(currentOffset/contentSize + dateMonths) toDate:minimumPage];
    ...
    //修改scrollView的位置
    NSInteger months = [NSDate getMonthsFrom:_minimumDate withToDate:date];
    [UIView animateWithDuration:0.5 animations:^{
        self.collectionView.contentOffset = TYSetPoint(months*self.collectionView.fs_width, 0);
    }];
}
    ...
    if (_scrollDirection != scrollDirection) {
        ...
        switch (_scope) {
            case FSCalendarScopeMonth: {
                ...
                _header.scrollDirection = _collectionViewLayout.scrollDirection == UICollectionViewScrollDirectionVertical?UICollectionViewScrollDirectionHorizontal:UICollectionViewScrollDirectionVertical;
...
- (CGFloat)preferedRowHeight
{
    if (_preferedRowHeight == FSCalendarAutomaticDimension) {
        ...
        if (!self.floatingMode) {
            switch (_scope) {
                case FSCalendarScopeMonth: {
                    _preferedRowHeight = (contentHeight-padding*2)/CollectionViewRowNum;
                    break;
                }
                case FSCalendarScopeWeek: {
    ...
    if (_scope == FSCalendarScopeWeek) {
        return _preferedRowHeight + 8;
    }
    return _notepadListCalendar ? 58.00/375*TYGetUIScreenWidth + 10 : 62.00/375*TYGetUIScreenWidth;
}
...
#pragma mark - Lyq添加
    if (!_rainCalendar) {
        ...
        if (_selectedDates.count) {
            ...
            _selectedDates = [[NSMutableArray alloc]initWithObjects:_selectedDates.firstObject, nil];
        }
    }
...
#ifdef KJGJFSCalendar_2_1_0
        JGJFSCalendarCell *cell = (JGJFSCalendarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
#else
...
- (void)selectDate:(NSDate *)date scrollToDate:(BOOL)scrollToDate forPlaceholder:(BOOL)forPlaceholder
{
    ...
    if (_collectionView.numberOfSections < targetIndexPath.section) {
        ...
        TYLog(@"section或者row超过了范围 _collectionView.numberOfSections = %@,targetIndexPath.section = %@",@(_collectionView.numberOfSections),@(targetIndexPath.section));
        return;
    }
    
    if ([_collectionView numberOfItemsInSection:targetIndexPath.section] < targetIndexPath.row) {
        
        TYLog(@"targetIndexPath.row = %@ ,collectionSection = %@",@(targetIndexPath.row),@([_collectionView numberOfItemsInSection:targetIndexPath.section]));
        return ;
    }
    ...
    if (forPlaceholder) {
        ...
        TYLog(@"targetIndexPath.section = %@,targetIndexPath.row = %@",@(targetIndexPath.section),@(targetIndexPath.row));
        [_collectionView selectItemAtIndexPath:targetIndexPath animated:YES scrollPosition:UICollectionViewScrollPositionNone];
#ifdef KJGJFSCalendar_2_1_0
        JGJFSCalendarCell *cell = (JGJFSCalendarCell *)[_collectionView cellForItemAtIndexPath:targetIndexPath];
#else
...
- (NSDate *)dateForIndexPath:(NSIndexPath *)indexPath scope:(FSCalendarScope)scope
{
    switch (scope) {
        case FSCalendarScopeMonth: {
            ...
            NSInteger currentWeekday = [self weekdayOfDate:currentPage];
            //Tony修改过的地方
            NSInteger numberOfPlaceholdersForPrev = ((currentWeekday- _firstWeekday) + 7) % 7 ?: 0;//(7 * !self.floatingMode);
            
            NSDate *firstDateOfPage = [self dateBySubstractingDays:numberOfPlaceholdersForPrev fromDate:currentPage];
            
            switch (_collectionViewLayout.scrollDirection) {
                case UICollectionViewScrollDirectionHorizontal: {
                    NSUInteger rows = indexPath.item % CollectionViewRowNum;
                    NSUInteger columns = indexPath.item / CollectionViewRowNum;
        ...
        case FSCalendarScopeMonth: {
            ...
            switch (_collectionViewLayout.scrollDirection) {
                case UICollectionViewScrollDirectionHorizontal: {
                    ...
                    item = columns*CollectionViewRowNum + rows;
                    break;
                }
                case UICollectionViewScrollDirectionVertical: {
...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        //Tony修改过的地方
        if (!_header && self.scope == FSCalendarScopeMonth) {
        ...
        if (_scrollEnabled) {
            if (!_deliver) {
                ...
                [_contentView insertSubview:deliver belowSubview:_header];
        ...
        if (!_weekdays.count) {
            ...
            for (int i = 0; i < weekSymbols.count; i++) {
                ...
                weekdayLabel.textColor = (i == 0)||(i == weekSymbols.count - 1)?JGJMainColor:_appearance.weekdayTextColor;//第一个和最后一个颜色不一样
...
- (void)invalidateWeekdaySymbols
{
    ...
    useVeryShortWeekdaySymbols = YES;
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.showTodayNotShowSubImage = _showTodayNotShowSubImage;
    cell.hidden = NO;
    cell.showHoldView = NO;
    cell.calendar = self;
    ...
    cell.subImageHidden = [self subImageForDate:cell.date];
    ...
    if (self.cc_selectedDate) {
        
        cell.selectedDate = self.cc_selectedDate;
    }
    
    if (self.moreDayCalendar) {
        
        // v3.5.2 cc添加
        BOOL isHaveSameDate = NO;
        JGJMoreDaySelectedModel *selectedModel;
        for (int i = 0; i < self.contractorTypeSelectedArray.count; i ++) {
            
            selectedModel = self.contractorTypeSelectedArray[i];
            if ([[selectedModel.date timestamp] isEqualToString:[cell.date timestamp]]) {
        ...
        JGJMoreDaySelectedModel *model = [[JGJMoreDaySelectedModel alloc] init];
        if (isHaveSameDate) {
            
            model.isSelected = YES;
            model.date = cell.date;
            model.type = selectedModel.type;
        
            cell.moreDaySelectedModel = model;
            
        }else {
            
            model.isSelected = NO;
            model.date = cell.date;
            model.type = self.contractorType;
            cell.moreDaySelectedModel = model;
...
#pragma mark -此处设置今天的样式
    if (_rainCalendar) {
        if (cell.dateIsSelected) {
            
            [self boardLayerRound:cell];
...
//        2.3.0版本的注释
        if (self.scope == FSCalendarScopeWeek) {
            ...
            cell.recordedImageView.hidden = YES;
            cell.backgroundColor = [UIColor whiteColor];
            ...
            if (cell.dateIsToday &&_scope == FSCalendarScopeMonth && !_rainCalendar) {
                
                UILabel *lable = [[UILabel alloc]initWithFrame:CGRectMake(0.6, 0, CGRectGetWidth(cell.frame) - 1.2, 2)];
                lable.backgroundColor = AppFontE44343DColor;
                lable.tag = 20;
            ...
            if (![NSString isEmpty:cell.subtitle] && !_rainCalendar) {
#pragma mark - 有数据的天数的勾勾
                cell.recordedImageView.hidden = NO;
                cell.placeView.layer.masksToBounds = YES;
                cell.placeView.layer.borderWidth = 1;
                cell.placeView.layer.borderColor = AppFontF9A00FColor.CGColor;
...
#if 1
            cell.hidden = cell.dateIsPlaceholder;//隐藏非本月的cell
         ...
         insetForSectionAtIndex:(NSInteger)section
{
    ...
    return UIEdgeInsetsMake(2.0f, 0.0f, 1.0f, 0.0f);
}
...
- (void)currentPageDidChange
{
#pragma mark - 几多人用来标记按钮状态
    if (self.selectShow) {//先移除选中数据
...
- (NSString *)subtitleForDate:(NSDate *)date
{
    ...
    return _ibEditing && _appearance.fakeSubtitles ? @"" : nil;
}
...
- (BOOL )subImageForDate:(NSDate *)date
{
    if (_dataSource && [_dataSource respondsToSelector:@selector(calendar:subImageForDate:)]) {
        return ![_dataSource calendar:self subImageForDate:date];
    }
    return YES;
}
...
- (NSDate *)findMonthForWeek:(NSDate *)week withRow:(NSInteger *)row
{
    ...
    if (rowNumber == -1) {
        ...
        NSDate *firstDateOfPage = [self dateBySubstractingDays:numberOfPlaceholdersForPrev fromDate:firstDayOfMonth];
        for (int i = 0; i < CollectionViewRowNum; i++) {
...
- (CGFloat )getCalendarHeaderHeight{
    return self.preferedHeaderHeight;
}

- (void)startanimationwithcell:(FSCalendarCell*)cell{
    
    UIView *holdview = [[UIView alloc]initWithFrame:CGRectMake(1, 1, CGRectGetWidth(cell.contentView.frame) - 4, CGRectGetHeight(cell.contentView.frame) - 4 )];
    
    UIImageView *imagview = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetWidth(cell.frame)/2-12, CGRectGetHeight(cell.frame)/2-12, 25, 20)];
    holdview.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.1];
    holdview.tag = 120;
    holdview.layer.borderWidth = 1;
    holdview.layer.cornerRadius = 4;
    holdview.layer.borderColor = AppFontd7252cColor.CGColor;
    holdview.clipsToBounds = YES;
    
    [cell addSubview:holdview];
    
    //    点工  包工
    imagview.image = [UIImage imageNamed:@"selected"];
    ...
    [UIView animateWithDuration:.2 animations:^{
        ...
        CGAffineTransformScale(imagview.transform, 1.2, 1.2);
        imagview.transform = newTransform;
    } completion:^(BOOL finished) {
        
        CGAffineTransform newTransform =
        CGAffineTransformScale(imagview.transform, 0.7, 0.7);
        imagview.transform = newTransform;
...
-(void)boardLayerRound:(FSCalendarCell *)cell
{
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 2, 0.5, CGRectGetHeight(cell.frame) - 2)];
    view.backgroundColor = AppFontccccccColor;
    view.tag = 1122;
    [cell addSubview:view];
    
    UIView *views = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(cell.frame) - .5, CGRectGetWidth(cell.frame), 0.5)];
    views.backgroundColor = AppFontccccccColor;
    views.tag = 1122;
    [cell addSubview:views];
    UIView *viewss = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetWidth(cell.frame) - 0.5, 2, 0.5, CGRectGetHeight(cell.frame) - 2)];
    viewss.backgroundColor = AppFontccccccColor;
    viewss.tag = 1122;
    [cell addSubview:viewss];
    
    
}

-(void)weekSelected:(UICollectionViewCell *)cell
{
//    UIView *weekholdview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(cell.frame), 2.5)];
//    UIView *view =[[UIView alloc]initWithFrame:CGRectMake(0, 2.5, CGRectGetWidth(cell.frame), CGRectGetHeight(cell.frame))];
//    view.alpha = .5;
//    view.tag = 119;
//    view.backgroundColor = AppFontf8d8d8Color;
//    weekholdview.backgroundColor = AppFontd7252cColor;
//    weekholdview.tag = 333;
//    [cell addSubview:weekholdview];
//    [cell addSubview:view];
}
-(void)weektoday:(UICollectionViewCell *)cell
{
//    UIView *weekholdview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(cell.frame), 2.5)];
//    UIView *view =[[UIView alloc]initWithFrame:CGRectMake(0, 2.5, CGRectGetWidth(cell.frame), CGRectGetHeight(cell.frame))];
//    view.alpha = .2;
//    view.tag = 130;
//    view.backgroundColor = AppFontd7252cColor;
//    weekholdview.backgroundColor = AppFontd7252cColor;
//    weekholdview.tag = 131;
//    [cell addSubview:weekholdview];
//    [cell addSubview:view];
}
...
- (NSMutableArray *)contractorTypeSelectedArray {
    
    if (!_contractorTypeSelectedArray) {
        
        _contractorTypeSelectedArray = [[NSMutableArray alloc] init];
    }
    return _contractorTypeSelectedArray;
}
