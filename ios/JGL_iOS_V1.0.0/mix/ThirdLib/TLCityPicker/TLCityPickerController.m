...
- (void)viewDidLoad {
    ...
    NSString *cityNameStr = [TYUserDefaults objectForKey:JLGSelectCityName];
    if (cityNameStr.length != 0) {//如果有数据才显示"返回"
        [self.leftButton addTarget:self action:@selector(cancelButtonDown:) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < 3) {
        ...
        if (indexPath.section == 0) {
            [cell setTitle:@"已经定位的城市"];
            [cell setCityArray:self.localCityData];
        }
        else if (indexPath.section == 1) {
            [cell setTitle:@"最近访问的城市"];
...
- (UISearchController *) searchController
{
    if (_searchController == nil) {
        ...
        [_searchController.searchBar setPlaceholder:@"请输入城市中文或拼音"];
        ...
        _searchController.hidesNavigationBarDuringPresentation = NO;//不隐藏navigationBar
        
        //项目特定需要
        UITextField *searchTextField = [[[_searchController.searchBar.subviews firstObject] subviews] lastObject];
        [searchTextField.layer setLayerBorderWithColor:TYColorHex(0xd9d9d9) width:1.0 radius:4.0];
        [_searchController.searchBar setImage:[UIImage imageNamed:@"searchBar"] forSearchBarIcon:UISearchBarIconSearch state:UIControlStateNormal];
    }
...
- (NSMutableArray *) localCityData
{
    if (_localCityData == nil) {
        ...
        if (self.locationCityID != nil) {
            ...
            if (city == nil) {
                TYLog(@"Not Found City: %@", self.locationCityID);
            }
            else {
