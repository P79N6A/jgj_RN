...
//        UIView *underLineView = [[UIView alloc] init];
        ...
        UIImageView *underLineView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:self.arrowIcon]];
        
        if (![NSString isEmpty:self.arrowIcon]) {
            
            underLineView.image = [UIImage imageNamed:self.arrowIcon];
            
            underLineView.contentMode = UIViewContentModeBottom;
        }
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (_titleColorGradientStyle == YZTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要
            ...
            labelX = _titleMargin + CGRectGetMaxX(lastLabel.frame) +  self.titleScrollViewX - 5;
            
            //yj添加titleScrollViewX
            
            if (i == 0 && self.titleScrollViewX > 0) {
                
                labelX = self.titleScrollViewX + CGRectGetMaxX(lastLabel.frame);
...
- (void)selectLabel:(UILabel *)label
{
    ...
    if (_selTitleFont) {
        ...
        label.font = _selTitleFont;
    }
    
    if (!_is_un_title_center) {
