...
- (UIButton *)sendBtn{
    if (!_sendBtn) {
        ...
        [rightBtn setTitleColor:JLGBlueColor forState:UIControlStateNormal];
        ...
        NSString *title = [NSString stringWithFormat:@"确认(%d)",0];
...
- (void)addNavBarCancelButton{
    ...
    button.frame = CGRectMake(0, 0, 48, 40.0f);
...
- (void)updateToolbar
{
    ...
    self.sendBtn.enabled = (count > 0);
    self.previewBtn.enabled = (count > 0);
    NSString *title = [NSString stringWithFormat:@"确认(%ld)",(long)count];
...
- (void) cancelBtnTouched{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:PICKER_TAKE_CANCEL object:nil];
    });
}
