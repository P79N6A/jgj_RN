...
@interface LGPhotoPickerBrowserViewController () <UIScrollViewDelegate,LGPhotoPickerPhotoScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,LGPhotoPickerCustomToolBarViewDelegate,UIActionSheetDelegate>
...
@property (nonatomic, strong) UIButton *editeBtn;// 开始绘制
@property (nonatomic, strong) UIButton *completeEdite;// 完成绘制
@property (nonatomic, strong) NSMutableArray *array;
@property (nonatomic, strong) JGJDrawingLineView *drawingView;
...
- (UIButton *)editeBtn {
    ...
    if (!_editeBtn) {
        ...
        _editeBtn.backgroundColor = [AppFont000000Color colorWithAlphaComponent:0.3];
        _editeBtn.frame = CGRectMake(self.view.size.width - 75, 25, 60, 30);
        [_editeBtn setTitle:@"编辑" forState:(UIControlStateNormal)];
        [_editeBtn setTitleColor:AppFontffffffColor forState:(UIControlStateNormal)];
        [_editeBtn setTitle:@"完成" forState:(UIControlStateSelected)];
        [_editeBtn setTitleColor:AppFontffffffColor forState:(UIControlStateSelected)];
        _editeBtn.titleLabel.font = FONT(AppFont34Size);
        _editeBtn.layer.cornerRadius = 3;

        [_editeBtn addTarget:self action:@selector(editePhoto:) forControlEvents:(UIControlEventTouchUpInside)];
    }
    return _editeBtn;
}

//- (UIButton *)completeEdite {
//
//    if (!_completeEdite) {
//
//        _completeEdite = [UIButton buttonWithType:UIButtonTypeCustom];
//        _completeEdite.backgroundColor = [AppFont000000Color colorWithAlphaComponent:0.3];
//        _completeEdite.frame = CGRectMake(self.view.size.width - 75, 55, 60, 30);
//        [_completeEdite setTitle:@"完成" forState:(UIControlStateNormal)];
//        [_completeEdite setTitleColor:AppFontffffffColor forState:(UIControlStateNormal)];
//        _completeEdite.titleLabel.font = FONT(AppFont34Size);
//        _completeEdite.layer.cornerRadius = 3;
//
//        [_completeEdite addTarget:self action:@selector(completeEditePhoto) forControlEvents:(UIControlEventTouchUpInside)];
//    }
//    return _completeEdite;
//}


- (void)editePhoto:(UIButton *)sender {
    
    self.collectionView.scrollEnabled = NO;
    if (sender.selected) {
        
        sender.selected = NO;
        [self completeEditePhoto];
        
    }else {
        
        sender.selected = YES;
        _drawingView = [[JGJDrawingLineView alloc] initWithFrame:CGRectMake(0, (self.view.size.height - self.currentImageSize.height) / 2.0, self.currentImageSize.width, self.currentImageSize.height)];
        
        LGPhotoPickerBrowserPhoto *photo = self.photos[_currentPage];
        self.drawingView.image = photo.photoImage;
        
        
        _drawingView.lineColor = AppFontEB4E4EColor;
        _drawingView.lineWidth = 2.f;
        [self.view addSubview:self.drawingView];
        [self.view bringSubviewToFront:self.editeBtn];
//        [self.view bringSubviewToFront:self.completeEdite];
    }
...
- (void)completeEditePhoto {
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowserCompleteBtnClickWithCurrentIndexPatch:image:)]) {
        
        [self.delegate photoBrowserCompleteBtnClickWithCurrentIndexPatch:_currentPage image:newImage];
    }
...
-(void)setImageSelectedIndex:(NSInteger)imageSelectedIndex{
    ...
    self.currentPage = imageSelectedIndex;
}
...
- (void)setupCollectionView{
    if (!_collectionView) {
        ...
        flowLayout.itemSize = CGSizeMake(self.view.size.width, self.view.size.height);
        ...
        self.currentPage = self.imageSelectedIndex;
    }
}
...
//    [self.view addSubview:self.completeEdite];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        //Tony修改过的
        photo = self.photos[indexPath.item >= self.photos.count?(self.photos.count - 1):indexPath.item];
        ...
        self.currentImageSize = CGSizeMake(scrollView.photoImageView.frame.size.width, scrollView.photoImageView.frame.size.height);
...
#pragma mark - 取消下面的图片显示 根据测试要求去掉张数
...
- (void)longPressAction {
    TYLog(@"long pressed");
#pragma mark - LYQ新增
    UIActionSheet *aler = [[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"保存到相册" otherButtonTitles:nil, nil];
    ...
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            if (_cellScrollView.photoImageView.image) {
                [self TYShowMessageWithText:@"保存成功"];
            }
        }else{
            if (_cellScrollView.photoImageView.image) {
                [self TYShowMessageWithText:@"没有用户权限,保存失败"];
            }
        }
    }
}

- (void)TYShowMessageWithText:(NSString *)text{
