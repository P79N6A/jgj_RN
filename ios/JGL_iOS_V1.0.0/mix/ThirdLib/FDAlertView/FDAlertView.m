...
- (UIButton *)deleteButton {
    ...
    if (!_deleteButton) {
        ...
        _deleteButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.contentView.frame) - 19, 5, 14, 14)];
        [_deleteButton setImage:[UIImage imageNamed:@"RecordWorkpoints_cellDelete"] forState:UIControlStateNormal];
...
- (void)initContentView {
    ...
    _contentView.center = self.center;
    [self addSubview:_contentView];
    [_contentView addSubview:self.deleteButton];
}
...
- (void)setMessageLabelOffset:(CGFloat)messageLabelOffset {
    
    _messageLabelOffset = messageLabelOffset;
    CGSize messageSize = [self getMessageSize];
    _messageLabel.frame = CGRectMake(MARGIN_LEFT_LARGE, _messageLabelOffset, MAX(contentViewWidth - MARGIN_LEFT_LARGE - MARGIN_RIGHT_LARGE, messageSize.width), messageSize.height+25 + 15);
}
...
- (void)initMessage {
    ...
    if (_message != nil) {
        ...
        CGSize messageSize = [self getMessageSize];
        _messageLabel.frame = CGRectMake(MARGIN_LEFT_LARGE, _titleView.frame.origin.y + _titleView.frame.size.height + SPACE_LARGE, MAX(contentViewWidth - MARGIN_LEFT_LARGE - MARGIN_RIGHT_LARGE, messageSize.width), messageSize.height+25 + 15);
...
- (void)initAllButtons {
    ...
    if (_buttonTitleArray.count > 0) {
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            }else if ([buttonTitle isEqualToString:@"我再想想"]) {
                
                [button setTitleColor:AppFont666666Color forState:UIControlStateNormal];
            }else if ([buttonTitle isEqualToString:@"我知道了"]) {
                
                button.backgroundColor = JGJMainColor;
...
- (void)show {
    ...
    [self showBackground];
//    [self showAlertAnimation];//这个效果太像安卓，去掉
#else
...
- (void)hide {
    ...
    if ([self.delegate respondsToSelector:@selector(alertViewHide)]) {
        
        [self.delegate alertViewHide];
    }
...
- (void)setButtonBackgoundColor:(UIColor *)color atIndex:(NSInteger)index {
