...
#define kHMScrollViewSelectionIndicatorH 2.5f
...
@property (nonatomic, strong) NSMutableArray *redPointLayersList;
@end
...
- (void)commonInit {
    ...
    self.selectionIndicatorHeight = kHMScrollViewSelectionIndicatorH;
...
- (void)setShowRedPointArray:(NSMutableArray *)showRedPointArray{
    ...
    if (!self.redPointLayersList.count) {
        return;
    }
    
    __weak typeof(self) weakSelf = self;
    [showRedPointArray enumerateObjectsUsingBlock:^(NSNumber *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx < weakSelf.redPointLayersList.count) {
            CALayer *layer = (CALayer *)weakSelf.redPointLayersList[idx];
            layer.hidden = ![obj integerValue];
        }
    }];
}
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic && self.customSegmentWidth) {
        size.width = self.customSegmentWidth;
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    [self.redPointLayersList removeAllObjects];
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat redPointY = rect.origin.y - redPointWH/2.0;
            CGFloat redPointX = rect.origin.x + MIN(  + rect.size.width/2 + 16.0,rect.size.width);
            
            CALayer *redPointLayer = [CALayer layer];
            redPointLayer.frame = CGRectMake(redPointX, redPointY, redPointWH, redPointWH);
            redPointLayer.backgroundColor = JGJMainColor.CGColor;
            redPointLayer.cornerRadius = redPointWH/2.0;
            
            if (idx < self.showRedPointArray.count) {
                redPointLayer.hidden = ![self.showRedPointArray[idx] integerValue];
            }else{
                redPointLayer.hidden = YES;
            }
            
            [self.scrollView.layer addSublayer:redPointLayer];
            
            [self.redPointLayersList addObject:redPointLayer];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            if (self.customIndicatorWidth) {
                sectionWidth = self.customIndicatorWidth;
            }
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                sectionWidth = [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue] - self.selectionIndicatorEdgeInsets.right;
                if (self.customIndicatorWidth) {
                    selectedSegmentOffset += sectionWidth/4 + 2.5;
                    sectionWidth = self.customIndicatorWidth;
                }
                
                return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, sectionWidth, self.selectionIndicatorHeight + self.selectionIndicatorEdgeInsets.bottom);
            }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView.contentOffset.x > self.subTextLayerWith) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(showLeftArrow:)]) {
            [self.delegate showLeftArrow:self];
        }
    } else if (((scrollView.contentOffset.x + scrollView.frame.size.width + self.subTextLayerWith) < scrollView.contentSize.width)) {
        
        if (self.delegate && [self.delegate respondsToSelector:@selector(showRightArrow:)]) {
            [self.delegate showRightArrow:self];
        }
    }
}
...
- (CGFloat)subTextLayerWith{
    if (!_subTextLayerWith) {
        __block CATextLayer *textLayer;
        [self.scrollView.layer.sublayers enumerateObjectsUsingBlock:^(CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[CATextLayer class]]) {
                textLayer = (CATextLayer *)obj;
                *stop = YES;
            }
        }];
        
        _subTextLayerWith = textLayer.frame.size.width;
    }
    
    return _subTextLayerWith;
}
...
- (NSMutableArray *)redPointLayersList
{
    if (!_redPointLayersList) {
        _redPointLayersList = [NSMutableArray array];
    }
    return _redPointLayersList;
}
