...
-(void)setupView{
    ...
    self.helpBtn.layer.borderColor = AppFont666666Color.CGColor;
    self.helpBtn.layer.borderWidth = 0.5;
    [self.helpBtn addTarget:self action:@selector(clickBtn) forControlEvents:UIControlEventTouchUpInside];
}
-(void)clickBtn
{
...
- (void)setChatListType:(JGJChatListType)chatListType{
    ...
    }else if (chatListType == JGJChatListRecord) {
...
- (NSDictionary *)chatTypeDic
{
    if (!_chatTypeDic) {
        _chatTypeDic = @{
                       ...
                       [NSString stringWithFormat:@"%@",@(JGJChatListRecord)]:@[@"暂无记录哦~",@"NoDataDefault_NoManagePro"]
                       };
    }
    return _chatTypeDic;
}
