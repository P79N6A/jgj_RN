...
- (void)setMineData{
    if ([NSString isEmpty:_topTimeModel.highlightString]) {//没有点击文字
        ...
        //单聊已读
        BOOL isChangeColor = [self.topTimeModel.chatMsgListModel.class_type isEqualToString:@"singleChat"] && (self.topTimeModel.chatMsgListModel.unread_members_num == 0);
...
//    self.rightLabel.text = [JGJTime acordingTimeStrRetrunMyTime:_topTimeModel.chatMsgListModel.send_time];
...
- (void)setOtherData{
    if ([NSString isEmpty:_topTimeModel.highlightString]) {//没有点击文字
        ...
        NSAttributedString *attributedString = [self addUnderLineWithStr:_topTimeModel.highlightString color:kChatListHighlightColor needUnderLine:YES];
