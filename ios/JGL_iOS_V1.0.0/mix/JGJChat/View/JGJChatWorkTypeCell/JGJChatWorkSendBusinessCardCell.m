...
- (void)setChatListModel:(JGJChatMsgListModel *)chatListModel {
    ...
    // 信息
    if ([self.chatListModel.recruitMsgModel.scale integerValue] == 0) {
        
        self.userInfoLabel.text = [NSString stringWithFormat:@"%@ 工龄 %@ 年",self.chatListModel.recruitMsgModel.nationality,self.chatListModel.recruitMsgModel.work_year];
    }else {
