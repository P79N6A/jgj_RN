...
- (void)commonSetInputView{
    self.inputView.placeholderFontSize = 15.0;
    ...
    self.inputView.ty_textHeightChangeBlock = ^(NSString *text, CGFloat textHeight) {
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(textHeightChangeWithCellHeight:)]) {
            
            [weakSelf.delegate textHeightChangeWithCellHeight:textHeight];
        }
    };
...
- (IBAction)noticeBtnClick:(id)sender {
    ...
    if (self.chatListType == JGJChatListNotice) {
        messageStr = @"仅有班组内成员会收到此通知";
    }else{
        messageStr = @"班组内的成员和其他与项目有关的人都会收到此报告";
    }
