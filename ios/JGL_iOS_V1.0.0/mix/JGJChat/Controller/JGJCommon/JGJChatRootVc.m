...
#import "JGJChatRootVc+ModifyInfoService.h"
...
- (JGJChatMsgListModel *)groupShowMsgModel:(JGJChatMsgListModel *)maxMsgModel {
    ...
    if ([firstVc isKindOfClass:[JGJChatListAllVc class]]) {
        ...
        if (firstVc.dataSourceArray.count > 0) {
            
            JGJChatMsgListModel *lastMsgModel = firstVc.dataSourceArray.lastObject;
            
            if (![NSString isEmpty:maxMsgModel.msg_id] && [NSString isEmpty:lastMsgModel.msg_id]) {
                
                if ([maxMsgModel.msg_id isEqualToString:lastMsgModel.msg_id]) {
            ...
            msgModel.group_id = lastMsgModel.group_id;
            
            msgModel.class_type = lastMsgModel.class_type;
            
            msgModel.msg_sender = lastMsgModel.msg_sender;
            
            msgModel.msg_type = lastMsgModel.msg_type;
            
            msgModel.send_time = lastMsgModel.send_time;
            
            JGJChatUserInfoModel *userInfo = [[JGJChatUserInfoModel alloc] init];
            
            userInfo.real_name = lastMsgModel.user_info.real_name;
            
            msgModel.user_info = userInfo;
            
            msgModel.sys_msg_type = lastMsgModel.sys_msg_type;
            
            //主要是msg_id不能改变，取最大的.聊聊列表显示用。msg_text

            msgModel.msg_id = maxMsgModel.msg_id;
            
            msgModel.msg_text = lastMsgModel.msg_text;
            
            msgModel.at_message = lastMsgModel.at_message;
            
            msgModel.title = lastMsgModel.title;
            
            msgModel.sendType = lastMsgModel.sendType;
        }
...
//            NSArray *existMsgs = [JGJChatMsgDBManger getChatMsgModel:maxMsgModel];
            ...
            if (existMsgs.count > 0) {
                ...
                if (existMsgs.count == 1) {
                    ...
                    if ([msgModel.msg_type isEqualToString:@"recall"] && [msgModel.msg_sender isEqualToString:myUid]) {
        ...
        }else {// 当前聊天窗口没有消息，直接添加新消息
            
            [weakself addMsglistModel:msgModel];
        }
...
- (void)socketRequestReadedToSender{
    ...
    shareSocket.messageReadedCallback = ^(JGJChatMsgListModel *readedMsgModel) {
        ...
        //用于未读人数小于当前的未读人数
        firstVc.readedMsgModel = readedMsgModel;
...
//    group_chat_icon
    ...
    NSString *imageStr = isSingleChat ? @"single_chat_icon" :  @"group_chat_icon";
    if (isSingleChat) {
...
- (void)backBtnClick:(id)sender{
    ...
    [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:NSClassFromString(@"JGJNewNotifyVC")] || [obj isKindOfClass:NSClassFromString(@"JGJMyChatGroupsVc")]) {
            popVC = obj;
            *stop = YES;
        }
    }];
    ...
    if (popVC) {
        ...
        //关掉 返回的JGJQRCodeVc、JGJJoinGroupVc这两个页面
...
//
//                if ([chatMsgListModel.picImage isKindOfClass:[UIImage class]]) {
//
...
//    }];
    ...
    for (NSInteger idx = 0; idx < baseVc.dataSourceArray.count; idx++) {
        
        JGJChatMsgListModel *chatMsgListModel = baseVc.dataSourceArray[idx];
        ...
        if (chatMsgListModel.chatListType == JGJChatListPic) {
            ...
            if (chatMsgListModel.picImage) {
                ...
                if ([chatMsgListModel.picImage isKindOfClass:[UIImage class]]) {
                    ...
                    [images addObject:chatMsgListModel.picImage];
                    
                    imageView.image = chatMsgListModel.picImage;
                ...
                if (chatMsgListModel.msg_src.count > 0) {
                    ...
                    [images addObject:[chatMsgListModel.msg_src firstObject]];
            ...
            if (indexPath.row == idx) {
                self.imageSelectedIndex = images.count - 1;
            }
        }
    }
...
//        [JGJCheckPhotoTool browsePhotos:self.imagesArray selImageViews:imageArray didSelPhotoIndex:self.imageSelectedIndex];
        
        [self.view endEditing:YES];
        
        [JGJCheckPhotoTool chatViewWebBrowsePhotoImageView:images selImageViews:imageArray didSelPhotoIndex:self.imageSelectedIndex];
...
- (UILabel *)titleLable {
    ...
    if (!_titleLable) {
        ...
        _titleLable.textColor = AppFont333333Color;
