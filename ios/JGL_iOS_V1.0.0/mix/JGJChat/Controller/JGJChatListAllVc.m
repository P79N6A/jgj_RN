...
//    if (self.chatInputView.switchingKeybaord) return;
    ...
    if ([NSString isFloatZero:yEndOffset] || yEndOffset == self.bottomViewConstraintB.constant || offset == self.bottomViewConstraintB.constant) {
        return;
    }
    ...
    switch (selectedIndex) {
        ...
        case 3:
        {
            ...
            if (belongType == JGJChatListBelongMine) {
                ...
                JGJMorePeopleViewController *morePepleContr = [[JGJMorePeopleViewController alloc]init];
                listModel.group_id = self.workProListModel.group_id;
                listModel.members_num = self.workProListModel.members_num;
                listModel.pro_id   = self.workProListModel.pro_id;
                listModel.pro_name = self.workProListModel.pro_name;
                listModel.group_name = self.workProListModel.group_name;
                NSLog(@" -- %@",self.workProListModel.group_name);
                morePepleContr.recordSelectPro = listModel;
                morePepleContr.isMinGroup = YES;
                nextVc = morePepleContr;
            }else{
...
//                mateReleaseBillVc.roleType = belongType == JGJChatListBelongMine?2:1;
//                mateReleaseBillVc.workProListModel = self.workProListModel;
//                nextVc = mateReleaseBillVc;
//                JGJQRecordViewController *recorVC = [[JGJQRecordViewController alloc]init];
                JGJMarkBillViewController *recorVC = [[UIStoryboard storyboardWithName:@"JGJMarkBillVC" bundle:nil]instantiateViewControllerWithIdentifier:@"JGJMarkBillVC"];

                recorVC.selectedDate = [NSDate date];
                recorVC.roleType = belongType == JGJChatListBelongMine?2:1;
                recorVC.workProListModel = self.workProListModel;
                recorVC.ChatType = YES;
                nextVc = recorVC;
            }
        }
            break;
        ...
        case 4:
        {
            JGJChatNoticeVc *chatNoticeVc= [[UIStoryboard storyboardWithName:@"JGJChat" bundle:nil] instantiateViewControllerWithIdentifier:@"JGJChatSignVc"];
            chatNoticeVc.signVcType = JGJChatSignVcUseType;
            chatNoticeVc.workProListModel = self.workProListModel;
...
- (void)setSuper_textView:(JGJChatMsgBaseCell *)baseCell{
