...
+ (BOOL)updateChatMessage:(JGJChatMsgListModel *)chatMsgListModel
{
    ...
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:chatMsgListModel];
    TYLog(@"查询  %@",[NSString stringWithFormat:@"UPDATE jgj_chatlist_data SET data = ,msg_type = %@,belongType = %@ ,local_id = %@ ,is_readed = %@ WHERE group_id = %@ AND msg_id = %@ AND token = %@ AND class_type = %@;",chatMsgListModel.msg_type,@(chatMsgListModel.belongType),chatMsgListModel.local_id,chatMsgListModel.is_readed,chatMsgListModel.group_id,chatMsgListModel.msg_id,chatMsgListModel.token,chatMsgListModel.class_type]);
    ...
    if (chatMsgListModel.belongType == JGJChatListBelongMine) {
        
        TYLog(@"SaveMEMEMEMEM========%@", chatMsgListModel.user_name);
        
    }else {
        
        TYLog(@"SaveOTherOther====== %@", chatMsgListModel.user_name);
