...
- (void)viewDidLoad {
    ...
    commonModel.classType = self.workProListModel.class_type;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case JGJGroupChatDetailInfoHeadCellType:{
            if (indexPath.row == 0) {
//                JGJGroupChatInfoMemberCell *memberCell  = [JGJGroupChatInfoMemberCell cellWithTableView:tableView];
//                memberCell.delegate = self;
//                memberCell.memberFlagType = ShowAddAndRemoveTeamMemberFlagType;
//                memberCell.memberModels = self.teamMemberModels;
//                JGJTeamMemberCommonModel *commonModel = [JGJTeamMemberCommonModel new];
//                commonModel.classType = self.workProListModel.class_type;
//                commonModel.memberType = JGJProMemberType;
...
//                teamCell.lineView.hidden = indexPath.row == 1;
        ...
        case JGJGroupChatDetailInfoMangerAuthorCellType:{
            ...
            if (indexPath.section == JGJGroupChatDetailInfoMangerAuthorCellType && indexPath.row == 0) {
                ...
                [teamCell.title markLineText:@"升级为工作中的班组，便于进行班组沟通管理" withLineFont:remarkTitleFont withColor:AppFont999999Color lineSpace:2.0];
...
- (NSArray *)handleGetRemoveMembers{
    ...
    NSArray *contacts = [self.checkAllMembers filteredArrayUsingPredicate:predicate];
    return contacts;
}
...
- (NSMutableArray *)detailCreatInfos {
    NSArray *allInfos = @[@[@"查看全部群成员"], @[@"群聊名称",@"群二维码"],@[@"我在本群的名字"],@[@"升级为班组\n升级为工作中的班组，便于进行班组沟通管理",@"转让管理权"]];
