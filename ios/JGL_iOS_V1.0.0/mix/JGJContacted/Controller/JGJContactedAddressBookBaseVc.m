...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return indexPath.section ==0 ? RowH : 55;
}
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            ...
            case FirstSectionGroupWorkCircleCellType:{
                ...
                workChatListVc.groupChatListVcType = JGJContactedAddressBookVcComeIn;
...
#pragma mark - 是否已查看朋友列表
- (void)setIsCheckFriendList:(BOOL)isCheckFriendList {
    
    _isCheckFriendList = isCheckFriendList;
    
    for (JGJConRecomVc *recomVc in self.navigationController.viewControllers) {
        
        if ([recomVc isKindOfClass:NSClassFromString(@"JGJConRecomVc")]) {
            
            recomVc.isCheckFreshFriend = YES;
...
//        searchResultView.chatType = JGJSingleChatType;
