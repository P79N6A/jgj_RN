//
//  JGJWorkbillCollectionViewCell.m
//  mix
//
...
@interface JGJWorkbillCollectionViewCell()
<
UITableViewDelegate,
...
>
@end
@implementation JGJWorkbillCollectionViewCell
...
- (void)awakeFromNib
{
    ...
    _BillTableview  = [[UITableView alloc]initWithFrame:CGRectMake(0, headerViewHeight, CGRectGetWidth(self.frame) , CGRectGetHeight(self.frame)  - headerViewHeight - 42.5)];
    _BillTableview.backgroundColor = AppFontfafafaColor;
    ...
    [self.contentView addSubview:_BillTableview];
    ...
    [_BillTableview addGestureRecognizer:tap];
...
-(void)initArr{
    ...
    if (JLGisLeaderBool || _morePeople) {
        ...
        _titleArr  = @[@[@"选择工人",@"选择日期"],@[@"填写金额",@"所在项目",@"记工备注"]];
        ...
        _titleArr    = @[@[@"班组长/工头",@"选择日期"],@[@"填写金额",@"所在项目",@"记工备注"]];
        
    }
    _imageArr = @[@[@"msjyb_icon_date_selected",@"msjyb_icon_person_selected"],@[@"msjyb_icon_price_selected",@"msjyb_icon_project_selected",@"msjyb_icon_record_selected"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section ==1&&indexPath.row == 0) {
        ...
        if (_yzgGetBillModel.totalNum) {
            cell.subTextFiled.text = _yzgGetBillModel.totalNum;
            cell.BigBool = YES;
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(didselectedBillTableviewforIndexpath:withTableviewclass:)]) {
        [self.delegate didselectedBillTableviewforIndexpath:indexPath withTableviewclass:NSStringFromClass([_BillTableview class])];
    }
}
...
- (void)ClickRefreshButton:(UIButton *)sender
{
    ...
    [self.BillTableview scrollToRowAtIndexPath:indexpath atScrollPosition:UITableViewScrollPositionBottom animated:YES];
}
-(void)ClicksaveButton:(UIButton *)sender
{
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(clickSaveDatabillButtonwithModel:)]) {
        [self.delegate clickSaveDatabillButtonwithModel:_yzgGetBillModel];
    }
...
-(void)setYzgGetBillModel:(YZGGetBillModel *)yzgGetBillModel
{
    ...
    if (!_yzgGetBillModel.totalNum ) {
        _NumLabled.text  = [NSString stringWithFormat:@"0.00 工资结算"];
        [self changeFontwithstr:_NumLabled.text withFont:[UIFont systemFontOfSize:20]];
    }
...
{
    if (tag == 10) {
        _yzgGetBillModel.totalNum = text;
        _NumLabled.text = text;
        _NumLabled.text  = [NSString stringWithFormat:@"%.2f 工资结算",[text floatValue]];
        [self changeFontwithstr:_NumLabled.text withFont:[UIFont systemFontOfSize:20]];
    }
}
