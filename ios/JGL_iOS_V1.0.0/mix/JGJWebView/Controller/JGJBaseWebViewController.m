...
- (void )viewDidLoad {
    ...
    [self.webView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_offset(_isHiddenTabbar ? 0 : -49);
    }];
...
- (UIBarButtonItem *)getLeftBarButton{
    ...
    CGRect leftButtonFrame = leftButton.frame;
    leftButtonFrame.size.width = leftButtonFrame.size.width/1.2*([[UIScreen mainScreen] bounds].size.width/320.0);
...
- (UIBarButtonItem *)getWhiteLeftBarButton{
    ...
    CGRect whiteLeftButtonFrame = whiteLeftButton.frame;
    whiteLeftButtonFrame.size.width = whiteLeftButtonFrame.size.width/1.2*([[UIScreen mainScreen] bounds].size.width/320.0);
    
    whiteLeftButton.frame = whiteLeftButtonFrame;
...
//        [TYShowMessage showHUBWithMessage:@"加载中,请稍候" WithView:self.webView]; //2.1.0去掉加载效果h5已做
    });
    ...
    TYLog(@"self.baseWebURL=== %@", self.baseWebURL);
    //加载地址数据
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    //状态栏的高度即可 聊天长按消息进来 不偏移
    if (!_chatLoneTouchComeIn) {
...
//    self.navigationController.title = [self.webView stringByEvaluatingJavaScriptFromString:@"document.title"];
    ...
    if([self.currentURL containsString:AboutUsURL]){//关于我们
        self.navigationController.title = @"关于我们";
    }else if([self.currentURL containsString:ContactUsURL]){//联系我们
        self.navigationController.title = @"联系我们";
    }else if ([self.currentURL containsString:DownLoadBillURL]) {
        self.navigationController.title = @"下载账单";
        
    }else if ([TYPredicate isCheckUrl:self.currentURL]) {
//标签栏会改变
...
//    [TYShowMessage hideHUD]; //2.1.0-yj注释掉因为会拦截加载
