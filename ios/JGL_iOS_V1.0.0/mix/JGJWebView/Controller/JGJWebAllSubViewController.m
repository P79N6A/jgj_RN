...
<
    ...
    JGJCustomShareMenuViewDelegate,
    JGJShareMenuViewDelegate,
...
//    [self registerCallBack];
    ...
    [self.webJSBridge registerHandler:@"getToken" handler:^(id data, WVJBResponseCallback responseCallback) {
...
-(instancetype)initWithWebType:(JGJWebType)webType URL:(NSString *)url{
    ...
    if (self) {
        ...
        //通过类型获取url
        self.baseWebURL = [self baseUrlWithWebType:webType URL:url];
...
- (NSURL *)baseUrlWithWebType:(JGJWebType)webType URL:(NSString *)url {
    ...
    return [NSURL URLWithString:baseWebURLString];
}
...
- (void)modifyUserAgent{
    ...
    NSString *client_info = [NSString stringWithFormat:@";person;ver=%@;infover=%@",currentVersion, @(infoVer)];
...
//            baseWebURLString = [baseWebURLString stringByAppendingString:MineInfoURL];
//  2.1.0修改
            baseWebURLString = MineInfoURL;
            break;
        case JGJWebTypePersonDetail:
            baseWebURLString = [baseWebURLString stringByAppendingString:PersonDetailSettingURL];
            break;
        case JGJWebTypeCollectionMsg:
            baseWebURLString = [baseWebURLString stringByAppendingString:CollectionMsgURL];
            break;
        case JGJWebTypeReceiveMsg:
            baseWebURLString = [baseWebURLString stringByAppendingString:ReceiveMsgURL];
            break;
        case JGJWebTypeCommentMsg:
            baseWebURLString = [baseWebURLString stringByAppendingString:CommentMsgURL];
            break;
        case JGJWebTypeContactUs:
            baseWebURLString = [JGJWebDiscoverURL stringByAppendingString:ContactUsURL];
            break;
        case JGJWebTypeSetting:
            baseWebURLString = [baseWebURLString stringByAppendingString:SettingURL];
            break;
        case JGJWebTypeSysMsgList:
            baseWebURLString = [baseWebURLString stringByAppendingString:SysMsgListUsURL];
            break;
        case JGJWebTypeCooperate:
            baseWebURLString = [baseWebURLString stringByAppendingString:CooperateURL];
            break;
        case JGJWebTypeMakeFriend:
            baseWebURLString = [baseWebURLString stringByAppendingString:[NSString stringWithFormat:@"%@%@",MakeFriendURL,[TYUserDefaults objectForKey:JLGSelectCityNo]]];
            break;
        case JGJWebTypeNewLife:
            {
//                NSDictionary *yiJiDic = [self getYiJiStr];
                
//                baseWebURLString = [NSString stringWithFormat:@"%@%@&preferably=%@&tabu=%@",NewLifeURL,[TYUserDefaults objectForKey:JLGSelectCityName],yiJiDic[@"yiStr"],yiJiDic[@"jiStr"]];
                
                baseWebURLString = NewLifeURL;
            }
            break;
        case JGJWebTypeNewLifeNews:
            baseWebURLString = NewLifeNewsURL;
            break;
        case JGJWebTypeComments:
            baseWebURLString = CommentsURL;
            break;
        case JGJWebTypeAgreement:
            baseWebURLString = [baseWebURLString stringByAppendingString:AgreementURL];
            break;
        case JGJWebTypeModifyPassword:
            baseWebURLString = [baseWebURLString stringByAppendingString:ModifyPasswordURL];
            break;
        ...
        case JGJWebTypePersonScore:{
            baseWebURLString = JGJWebScoreWebURL;
        }
            break;
        case JGJWebTypeFindJob:
            baseWebURLString = JGJWebFindJobURL;
            break;
        case JGJWebTypeRecruitJob: {
            baseWebURLString = JGJWebRecruitURL;
        }
            break;
        ...
        case JGJWebTypeMyProExp:{
            baseWebURLString = JGJWebMyProExpURL;
        }
            break;
        ...
        case JGJWebTypDefaultFindProType: {
            
            baseWebURLString = [NSString stringWithFormat:@"%@%@",JGJWebDiscoverURL,JGJDefaultFindProURL];
            
        }
        case JGJWebTypeComplain:
        {
            baseWebURLString = [JGJWebDiscoverURL stringByAppendingString:url];
        }
...
- (void )setBaseWillAppear{
    ...
    if ([[self.baseWebURL absoluteString] rangeOfString:PersonDetailSettingURL].location !=NSNotFound) {//设置
        [self setNavigationBarTransformProgress:YES];
    }else if ([[self.baseWebURL absoluteString] rangeOfString:MineInfoURL].location !=NSNotFound){//个人设置
        //不做操作
    }
    else {
...
//            topView.backgroundColor = AppFontfafafaColor;
...
- (void)setWebTitle:(NSString *)url{
    ...
    if ([self.currentURL containsString:CommentsURL]) {
        self.title = @"意见反馈";
    }else if ([self.currentURL containsString:@"is_demo=1"]){//统计
...
- (void)registerCallBack{
    ...
    [self setNavigationBarTransformProgress:NO];//基本都要显示顶部的
    
    [self setStatusBarStyle:UIStatusBarStyleDefault];
    
    self.webView.dataDetectorTypes = UIDataDetectorTypeAll;
    if (([self.currentURL rangeOfString:SettingURL].location !=NSNotFound)) {//设置
        [self exitLoginCallBack];
    }else if(([self.currentURL rangeOfString:NewLifeURL].location !=NSNotFound)){//新生活
        [self setStatusBarStyle:UIStatusBarStyleDefault];

        [self setNavigationBarTransformProgress:YES];
    }else if(([self.currentURL rangeOfString:MineInfoURL].location !=NSNotFound)){//我的资料
        [self MineInfoCallBack];
        [self setNavigationBarTransformProgress:YES]; //隐藏头部
        ...
        [self setNavigationBarTransformProgress:YES]; //隐藏头部
    }else if (([self.currentURL rangeOfString:PersonDetailSettingURL].location !=NSNotFound) || ([self.currentURL rangeOfString:AboutUsURL].location !=NSNotFound)) {//个人主页
        [self PersonDetailBack];
        [self setNavigationBarTransformProgress:YES];//个人主页不用显示返回
    }else if ([self.currentURL rangeOfString:MakeFriendWantToSayURL].location !=NSNotFound){//拍照
        [self phoneJsCallBack];
    ...
    }else if([self.currentURL rangeOfString:WebProjectList].location !=NSNotFound){
        ...
        [self setNavigationBarTransformProgress:YES];//招聘不用显示返回(2.1.0-yj)
    }else if([self.currentURL rangeOfString:JGJWebFindJobURL].location !=NSNotFound && self.webType == JGJWebTypeFindJob){
        [self projectCallBack];
        [self setNavigationBarTransformProgress:YES];//找工作不用显示返回(2.1.0-yj)
        [self bottomShowTabBar:YES];//解决找工作底部高度问题
    }else if([self.currentURL rangeOfString:JGJWebRecruitURL].location !=NSNotFound && self.webType == JGJWebTypeRecruitJob){
        [self projectCallBack];
        [self setNavigationBarTransformProgress:YES];//招聘不用显示返回(2.1.0-yj)
        [self bottomShowTabBar:YES];//临时添加解决招聘高度问题
    }else if ([self.currentURL rangeOfString:JGJWebScoreWebURL].location !=NSNotFound) { //我的积分
        [self setNavigationBarTransformProgress:YES];//隐藏头子(2.1.0-yj) 隐藏底部
    }else if ([self.currentURL rangeOfString:JGJWebShoppingMal].location != NSNotFound) { //积分商城
        [self setNavigationBarTransformProgress:YES];//隐藏头子(2.1.0-yj)，隐藏底部
    }else if ([self.currentURL rangeOfString:WebNewLifeStr].location != NSNotFound) { //咨询
        [self setNavigationBarTransformProgress:YES];//隐藏头子(2.1.0-yj)，隐藏底部
        //注册头子
        [self projectCallBack];
    }else if (self.webType == JGJWebTypeDynamic || self.webType == JGJWebTypeMyCollection) {
        [self setNavigationBarTransformProgress:YES];//隐藏头子(2.1.0-yj)，隐藏底部
    }else if (self.webType == JGJWebTypeMyProExp) {
        
        [self setNavigationBarTransformProgress:YES];
    }else if (self.webType == JGJWebTypeProDetailType) {
    
        [self setNavigationBarTransformProgress:YES];
    }else if (self.webType == JGJWebTypePersonScore) {
    
        [self setNavigationBarTransformProgress:YES];
    }else if (self.webType == JGJWebTypeSendPushType){
    
        [self setNavigationBarTransformProgress:YES];
    }
    
    //推送分享账单 去掉顶部导航栏
    if (self.webType == JGJWebTypeSendPushType){
    ...
    }else if (self.webType == JGJWebTypDefaultFindProType) {
        
        [self setNavigationBarTransformProgress:YES];
        
    }
    
    if (self.webType == JGJWebTypeInnerURLType) {
...
//    //分享
//    if ([self.currentURL containsString:DownLoadBillURL]) {
//        //分享
//        [self downLoadBillCallBack];
//    }else {
...
- (void)callHandleInfo:(NSDictionary *)info {
    ...
    NSString *sharejson = [NSString getJsonByData:info];
    
    [self.webJSBridge callHandler:@"shareDynamicComment" data:sharejson responseCallback:^(id responseData) {
        ...
        if (data) {
            if ([data[@"name"] isEqualToString:@"YZGGetIndexRecordViewController"]) {
                ...
                if ([data[@"date"] length]) {
                    ...
                    NSString *timeDate = data[@"date"];
                    date=[formatter dateFromString:timeDate];
                }else{
                    date = [NSDate date];
                }
                getIndexRecordVc.sendType = YES;

                getIndexRecordVc.date = date;
                [self.navigationController pushViewController:getIndexRecordVc animated:YES];
                
            }else if ([data[@"name"] isEqualToString:@"connection"]) { //人脉推荐
                
                [self conRecomVc];
            }else if ([data[@"name"] isEqualToString:@"identity"]) {
                
                [self changeRole];
                
            }else if ([data[@"name"] isEqualToString:@"JGJKnowRepoVc"]) {
                
                [self checkKnowRepoVc];
                
            }else if ([data[@"name"] isEqualToString:@"JGJDredgeWeChatServiceViewController"]) {
                
                self.responseCallback = responseCallback;
                
                [self gotoDredgeWeChatServiceViewController];
...
- (void)checkKnowRepoVc {
    ...
    knowBaseVc.knowRepoVcBackBlock = ^{
...
- (void)gotoDredgeWeChatServiceViewController {
    
    JGJDredgeWeChatServiceViewController *dregeVC = [[JGJDredgeWeChatServiceViewController alloc] init];
    dregeVC.isWebViewComeIn = YES;
    [self.navigationController pushViewController:dregeVC animated:YES];
    
    TYWeakSelf(self);
    dregeVC.dredgeWeChatSuccess.getRedPacket = ^{
...
- (void)conRecomVc {
    ...
    JGJConRecomVc *recomVc = [JGJConRecomVc new];
...
//    BOOL isShowHeader = [self.currentURL containsString:JLGHttpRequest_WX2] || [self.baseWebURL.absoluteString containsString:JLGHttpRequest_WX2] ;
    if (self.webType == JGJWebTypeExternalThirdPartBannerType) {
        self.navigationController.navigationBar.hidden = NO;
        self.webView.frame = self.view.bounds;
        [self.webView mas_updateConstraints:^(MASConstraintMaker *make) {
...
#pragma mark - 点击我的积分购买之后的分享
- (void)handleRegisterMyScoreShare {
    
    NSString *sharejson = [NSString getJsonByData:@{@"share":@"1"}];
    [self.webJSBridge callHandler:@"webLink" data:sharejson responseCallback:^(id responseData) {
...
//        }
        if (chatModel.is_chat) {
            ...
            JGJAddFriendSendMsgVc *sendMsgVc = [weakSelf handleAddFriendWithChatModel:chatModel];
...
- (void)handleRegisterAppSet {
    ...
    [self.webJSBridge registerHandler:@"appSet" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        settingVc.mineSettingVcBackBlock = ^{
...
- (void)LoadWebViewFailure{
    if (![self.baseWebURL.absoluteString containsString:GamePlatform1758URL] || ![self haveThirdWebAddress]) {
        //如果不是游戏的界面，才需要显示失败
...
- (void)getVersionIOSCallBack{
    ...
    NSString *currentVersion = [NSString stringWithFormat:@"(\'%@\')",versionNum];
...
- (void)registerFooter {
    ...
    [self.webJSBridge registerHandler:@"footerController" handler:^(id data, WVJBResponseCallback responseCallback) {
        if ([data[@"state"] isEqualToString:@"hide"]) {
            ...
            if (self.isRecruitmentSituationMsgComeIn) {
...
- (void)projectListCallBack{
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"同步项目" style:UIBarButtonItemStylePlain target:self action:@selector(handleSynBillButtonPressed:)];
...
- (void)statisticsCallBack{
    ...
    __weak typeof(self) weakSelf = self;
    [self.webJSBridge registerHandler:@"syncBill" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        wageMoreDetailInitModel.infoId = [data[@"pid"] integerValue]; 
        
        wageMoreDetailInitModel.name = data[@"pro_name"];
    ...
    [self.webJSBridge registerHandler:@"checkBill" handler:^(NSDictionary *data, WVJBResponseCallback responseCallback) {
        ...
        if (data[@"url"]) {
            ...
            JGJWebAllSubViewController *webVc = [[JGJWebAllSubViewController alloc] initWithUrl:url];
...
- (void)commentsCallBack{
    ...
    [recordDic setValue:[NSBundle mainBundle].infoDictionary[@"CFBundleShortVersionString"] forKey:@"ver"];//app版本
...
- (void)handleSynBillButtonPressed:(id)sender{
    
    JGJSynBillingCommonModel *synBillingCommonModel = [[JGJSynBillingCommonModel alloc] init];
    JGJSynBillingManageVC *synBillingManageVC = [[UIStoryboard storyboardWithName:@"JGJSynBilling" bundle:nil] instantiateViewControllerWithIdentifier:@"JGJSynBillingManageVC"];
    synBillingCommonModel.synBillingTitle = @"需同步项目的联系人";
    synBillingCommonModel.isWageBillingSyn = YES;
    synBillingManageVC.synBillingCommonModel = synBillingCommonModel;
    self.navigationController.navigationBar.hidden = NO;
    synBillingManageVC.skipToNextVc = ^(UIViewController *vc){
...
//        TYLog(@"webViewFrame = %@",NSStringFromCGRect(webViewFrame));
#else
        //设置webview的frame
...
- (void)findCallBack{
    ...
    [self.webJSBridge registerHandler:@"showCalendar" handler:^(id data, WVJBResponseCallback responseCallback) {
        __strong typeof(weakSelf) weakStrongSelf = weakSelf;
        UIViewController *JGJCalendarViewController = [[UIStoryboard storyboardWithName:@"YZGHomeStorBoard" bundle:nil] instantiateViewControllerWithIdentifier:@"JGJCalendarViewController"];
        [weakStrongSelf.navigationController pushViewController:JGJCalendarViewController animated:YES];
    }];
    ...
    [self.webJSBridge registerHandler:@"appLink" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        if ([data[@"title"] isEqualToString:@"玩游戏"]) {
            ...
            if (![weakStrongSelf checkIsLogin]) {
                return ;
        ...
        if ([url rangeOfString:JGJWebDomainURL].location != NSNotFound && [url rangeOfString:@"topdisplay=1"].location != NSNotFound) {
            ...
            //            webVc = [[JGJWebAllSubViewController alloc] initWithUrl:[NSURL URLWithString:data[@"url"]]];
...
//        weakSelf.tabBarController.selectedIndex = 0;
        ...
        JGJMyWorkCircleProListModel *proListModel = [JGJMyWorkCircleProListModel mj_objectWithKeyValues:data];
...
- (void)registerLogin {
    ...
    [self.webJSBridge registerHandler:@"login" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        loginVC.backVc = self;
        __weak typeof(self) weakSelf = self; //2.1.0 -yj登录进来刷新页面
...
//    JGJWebAllSubViewController *recordWebViewVc = [[JGJWebAllSubViewController alloc] initWithUrl:[NSURL URLWithString:[StatisticsDemoWebURL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
    JGJWebAllSubViewController *recordWebViewVc = [[JGJWebAllSubViewController alloc] initWithWebType:JGJWebTypeExternalThirdPartBannerType URL:StaticDemoWebURL];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    //返回项目类型
    __block NSString *jsFunctStr;
    
    if ([self.currentURL rangeOfString:MineInfoURL].location !=NSNotFound) {//如果是修改资料
        [JLGHttpRequest_AFN uploadImageWithApi:@"jlsignup/modifyheadpic" parameters:nil image:image success:^(NSDictionary *responseObject) {
            jsFunctStr= [NSString stringWithFormat:@"getNewHeadPicture('%@')",responseObject[@"imgpath"]];
            dispatch_async(dispatch_get_main_queue(), ^{
#pragma mark - 此处造成异常崩溃 故更换js交互方法 LYQ
//                [self.context evaluateScript:jsFunctStr];
    ...
    }else if([self.currentURL rangeOfString:MakeFriendWantToSayURL].location !=NSNotFound){//如果是交朋友
        jsFunctStr= [NSString stringWithFormat:@"getPhotoStream('%@','%@')",imagebase64,imageType];
        dispatch_async(dispatch_get_main_queue(), ^{
//            [self.context evaluateScript:jsFunctStr];
            [self.webView stringByEvaluatingJavaScriptFromString:jsFunctStr];
...
//- (JGJCustomShareMenuView *)shareMenuView {
//    if (!_jgjShareBillView) {
//        _shareMenuView = [[JGJCustomShareMenuView alloc] initWithFrame:TYGetUIScreenRect];
//        _shareMenuView.delegate = self;
//        _shareMenuView.Vc = self;
//    }
...
//
//    [self handleRegisterMyScoreShare];
//}
//
///**
// 关闭当前页面之后
...
//    province = @"四川省";
    ...
    [self.webJSBridge registerHandler:@"getLocation" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        TYLog(@"callBackDic ==== %@  localJson === %@", callBackDic, localJson);
...
- (void)registerCheckPerson {
    ...
    [self.webJSBridge registerHandler:@"returnPerson" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        if ([data isKindOfClass:NSDictionary.class]) {
            ...
            if ([page isEqualToString:@"job"]) {
                [JGJDataManager sharedManager].addFromType = JGJFriendAddFromFindJobs;
            } else if ([page isEqualToString:@"dynamic"]) {
                [JGJDataManager sharedManager].addFromType = JGJFriendAddFromWorkmateCommunity;
            } else if ([page isEqualToString:@"connection"]) {
                [JGJDataManager sharedManager].addFromType = JGJFriendAddFromConnection;
            }
        }
...
#pragma mark - 显示分享到不同类型

//分享代码注释

- (void)registerShareToType {
    
//    TYWeakSelf(self);
//
//    [self.webJSBridge registerHandler:@"shareTo" handler:^(id data, WVJBResponseCallback responseCallback) {
//
//        NSDictionary *dic = (NSDictionary *)data;
//
//        JGJCustomShareMenuView *shareMenuView = [[JGJCustomShareMenuView alloc] init];
...
//        JGJShowShareMenuModel *shareModel = [JGJShowShareMenuModel mj_objectWithKeyValues:dic];
//
//        NSArray *snsNames = @[UMShareToWechatTimeline, UMShareToWechatSession, UMShareToQQ];
//
//        NSString *snsName = UMShareToWechatTimeline;
//
//        if (shareModel.type <= 3 && shareModel.type > 0) {
//
//            snsName = snsNames[shareModel.type-1];
//        }
//
//        shareMenuView.shareMenuViewType = JGJShareMenuViewQQType;
//
//        [shareMenuView shareInVc:self linkUrl:shareModel.url?:@"" snsName:snsName text:shareModel.describe?:@"" imageUrl:shareModel.imgUrl];
//
//    }];
...
- (BOOL)forbidShowTabBar {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        ...
        if ([vc isKindOfClass:NSClassFromString(@"JGJTabBarViewController")]) {
...
#pragma mark - 招聘或者没有项目使用
- (void)requestWebviewWithWebType:(JGJWebType)webType{
    
    self.webType = webType;
    
    self.baseWebURL = [self baseUrlWithWebType:webType URL:nil];
