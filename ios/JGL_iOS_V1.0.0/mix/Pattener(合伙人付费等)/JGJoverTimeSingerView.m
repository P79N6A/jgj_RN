...
+(void)showOverTimeViewWithModel:(JGJOverTimeModel *)timeModel andClickIkwonButton:(IkownBlock)cancelBlock
{
    ...
    if (timeModel.blackText) {
        timeView.contentLable.textColor = AppFont333333Color;
    }
    if (timeModel.singerTpl) {
        timeView.contentLable.text = timeModel.contentStr;
        [timeView.IkownButton setTitle:timeModel.buttonTitle forState:UIControlStateNormal];
        [timeView.contentLable markText:timeModel.redstr?:@"" withColor:AppFontEB4E4EColor];
