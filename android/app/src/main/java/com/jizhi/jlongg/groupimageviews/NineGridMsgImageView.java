 ...
 */
public class NineGridMsgImageView<T> extends ViewGroup {
    ...
    public NineGridMsgImageView(Context context) {
        this(context, null);
    }

    public NineGridMsgImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NineGridImageView);
        this.mGap = (int) typedArray.getDimension(R.styleable.NineGridImageView_imgGap, 10);
        this.mSingleImgSize = typedArray.getDimensionPixelSize(R.styleable.NineGridImageView_singleImgSize, 180);
     ...
     * @param lists 图片数据集合
     */
    public void setImagesDataXj(List lists) {
        if (lists == null || lists.isEmpty()) {
            this.setVisibility(GONE);
            return;
        } else {
        ...
        int[] gridParam = calculateGridParamXj(lists.size(), mShowStyle);
        mRowCount = gridParam[0];
        mColumnCount = gridParam[1];
        if (mImgDataList == null) {
            int i = 0;
            ...
            if (mAdapter != null) {
                ...
                imageView.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        mAdapter.onItemImageClick(getContext(), position, mImgDataList, imageView);
                    }
                });
                return imageView;
            } else {
     ...
     * @return 宫格参数 gridParam[0] 宫格行数 gridParam[1] 宫格列数
     */
    protected static int[] calculateGridParamXj(int imagesSize, int showStyle) {
        int[] gridParam = new int[2];
        switch (showStyle) {
            case STYLE_FILL:
                if (imagesSize < 3) {
            ...
            default:
            case STYLE_GRID:
                gridParam[0] = imagesSize / 4 + (imagesSize % 4 == 0 ? 0 : 1);
                gridParam[1] = 4;
        }
        return gridParam;
    }
