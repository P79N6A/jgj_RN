 ...
 * 作者:xuj
 */
public class MemberAccountPersonRecyclerViewAdapter extends RecyclerView.Adapter<MemberAccountPersonRecyclerViewAdapter.ViewHolder> {
    ...
    public MemberAccountPersonRecyclerViewAdapter(Context context) {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        switch (getItemViewType(position)) {
            case DATA: //正常的数据
                PersonBean bean = selectList.get(position);
                if (isShowUserName) {
                    holder.name.setVisibility(View.VISIBLE);
                    holder.name.setText(NameUtil.setName(bean.getName()));
                } else {
    ...
    public void addData(PersonBean info) {
        if (selectList == null) {
    ...
    public void removeDataByTelephone(String telephone) {
        ...
        for (int i = 0; i < size; i++) {
            if (telephone.equals(selectList.get(i).getTelph())) {
                remove(i);
                return;
            }
        }
    ...
    public List<PersonBean> getSelectList() {
        return selectList;
    }

    public void setSelectList(List<PersonBean> list) {
        if (selectList == null) {
            selectList = new ArrayList<>();
        }
