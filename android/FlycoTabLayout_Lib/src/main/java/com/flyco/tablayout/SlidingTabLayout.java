...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    public SlidingTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setClipChildren(false); //android:clipChildren的意思：是否限制子View在其范围内
     ...
     * 更新数据
     */
    public void updateTitle(int updatePosition, String updateTitle) {
        if (mTabsContainer != null && mTabsContainer.getChildCount() > 0) {
            View tableView = mTabsContainer.getChildAt(updatePosition);
            TextView textView = (TextView) tableView.findViewById(R.id.tv_tab_title);
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
     ...
     * @param matchesText    需要变化的字符
     */
    public void fillTextRed(String completeString, String matchesText) {
        if (TextUtils.isEmpty(completeString)) {
            return;
        }
        if (mTabsContainer.getChildCount() > 1) {
            View tableView = mTabsContainer.getChildAt(1);
            TextView titleText = (TextView) tableView.findViewById(R.id.tv_tab_title);
            if (TextUtils.isEmpty(matchesText)) {
                titleText.setText(completeString);
                return;
            }
            titleText.setText(Html.fromHtml("<font >" + completeString + "</font><font color='#d7252c'>" + matchesText + "</font>"));
        }
    }
//    /**
//     * 更新数据
